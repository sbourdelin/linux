				...
				int expected_errno, const char *errno_str)
{
	...
	snprintf(msg, sizeof(msg), "Check failure of execveat(%d, '%s', %d) with %s...\n",
				     ...
				     int expected_rc, int expected_rc2)
{
	...
	if (pathlen > 40)
		snprintf(msg, sizeof(msg), "Check success of execveat(%d, '%.20s...%s', %d)...\n",
			fd, path, (path + pathlen - 20), flags);
	else
		snprintf(msg, sizeof(msg), "Check success of execveat(%d, '%s', %d)...\n",
			fd, path?:"(null)", flags);
	child = fork();
	if (child < 0) {
		ksft_test_result_fail(msg);
