...
int bpf_sk_lookup_test0(struct __sk_buff *skb)
{
	...
	sk = bpf_sk_lookup_tcp(skb, tuple, tuple_len, BPF_CURRENT_NETNS, 0);
	if (sk)
		bpf_sk_release(sk);
...
int bpf_sk_lookup_test1(struct __sk_buff *skb)
{
	...
	sk = bpf_sk_lookup_tcp(skb, &tuple, sizeof(tuple), BPF_CURRENT_NETNS, 0);
	if (sk)
		bpf_sk_release(sk);
	return 0;
}
...
int bpf_sk_lookup_test2(struct __sk_buff *skb)
{
	...
	bpf_sk_lookup_tcp(skb, &tuple, sizeof(tuple), BPF_CURRENT_NETNS, 0);
	return 0;
}
