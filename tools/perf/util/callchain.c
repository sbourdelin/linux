...
fill_node(struct callchain_node *node, struct callchain_cursor *cursor)
{
	...
	while (cursor_node) {
		...
		if (cursor_node->branch) {
			...
			call->removed_loop = cursor_node->removed_loop;
		}
				     ...
				     struct callchain_list *cnode)
{
	...
	if (left == right) {
		if (node->branch) {
			...
			cnode->removed_loop += node->removed_loop;
		}
				   ...
				   int removed_loop)
{
	...
	if (removed_loop) {
		scnprintf(removed_str, sizeof(removed_str),
			 ", removed loops:%d", removed_loop);
		str = removed_str;
	} else
		str = (char *)null_str;
					...
					FILE *fp, char *bf, int bfsize)
{
	...
	if (node) {
		...
		list_for_each_entry(call, &node->val, list)
			removed_loop += call->removed_loop;
	}

	return callchain_counts_printf(fp, bf, bfsize, branch_count,
				       predicted_count, abort_count,
				       cycles_count, removed_loop);
}
