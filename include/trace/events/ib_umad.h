...
DECLARE_EVENT_CLASS(ib_umad_template,
	...
	TP_STRUCT__entry(
		...
		__field(u64,            tid)
		__array(u8,             gid, 16)
		__string(dev_name, file->port->ib_dev->name);
		__field(u8,             traffic_class)
	),

	TP_fast_assign(
		__assign_str(dev_name, file->port->ib_dev->name);
	...
	TP_printk("%s:%d umad_hdr: id 0x%08x status 0x%08x ms %u ret %u len %u QP%u " \
