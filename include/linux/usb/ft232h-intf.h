...
/*
 * The FTDI protocol functions, mainly borrowed from libftdi
 */
int ftdi_ctrl_xfer(struct usb_interface *intf, struct ctrl_desc *desc);
int ftdi_bulk_xfer(struct usb_interface *intf, struct bulk_desc *desc);
int ftdi_read_data(struct usb_interface *intf, void *buf, size_t len);
int ftdi_write_data(struct usb_interface *intf, const char *buf, size_t len);
int ftdi_set_baudrate(struct usb_interface *intf, int baudrate);
int ftdi_set_bitmode(struct usb_interface *intf, unsigned char bitmask,
		     unsigned char mode);
int ftdi_disable_bitbang(struct usb_interface *intf);
 ...
 * in FIFO-FPP, MPSSE-SPI or MPSSE-I2C drivers
 */
