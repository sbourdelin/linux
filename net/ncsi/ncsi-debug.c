...
{
	return single_open(file, ncsi_dev_pkt_seq_show, PDE_DATA(inode));
}
...
int ncsi_dev_init_debug(struct ncsi_dev_priv *ndp)
{
	...
	ndp->stats.pde = proc_create_data("stats", 0400, ndp->pde,
					  &ncsi_dev_stats_fops, ndp);
	ndp->pkt.pde = proc_create_data("pkt", 0600, ndp->pde,
					&ncsi_dev_pkt_fops, ndp);
...
{
	if (ndp->pkt.pde)
		proc_remove(ndp->pkt.pde);
	if (ndp->stats.pde)
		proc_remove(ndp->stats.pde);
	if (ndp->pde)
		proc_remove(ndp->pde);
}
