...
megasas_fusion_update_can_queue(struct megasas_instance *instance, int fw_boot_context)
{
	...
	if (instance->is_ventura)
		instance->max_mpt_cmds = instance->max_fw_cmds * RAID_1_10_RMW_CMDS;
	else
...
megasas_alloc_cmdlist_fusion(struct megasas_instance *instance)
{
	 ...
	 */
	fusion->cmd_list =
		kmalloc(sizeof(struct megasas_cmd_fusion *) * max_mpt_cmd, GFP_KERNEL);
	if (!fusion->cmd_list) {
		dev_err(&instance->pdev->dev,
			"Failed from %s %d\n",  __func__, __LINE__);
		return -ENOMEM;
	 ...
	 memset(fusion->cmd_list, 0,
		sizeof(struct megasas_cmd_fusion *) * max_mpt_cmd);
			  ...
			  struct megasas_cmd_fusion *cmd)
{
	...
	if (fp_possible) {
		...
		cmd->isRaid_1_fp_write = io_info.isRaid_1_fp_write;
		if (io_info.isRaid_1_fp_write)
	...
	struct megasas_cmd_fusion *cmd, struct scsi_cmnd *scmd)
{
	...
	if (cmd->io_request->RaidContext.raid_context_g35.numSGE >
		fusion->max_sge_in_main_msg)
			    ...
			    struct megasas_cmd_fusion *r1_cmd)
{
	...
	if (r1_cmd) {
		...
		req_desc2 = megasas_get_request_descriptor(instance, r1_cmd->index-1);
		if (req_desc2) {
			req_desc2->Words = 0;
...
complete_cmd_fusion(struct megasas_instance *instance, u32 MSIxIndex)
{
	...
	while (d_val.u.low != cpu_to_le32(UINT_MAX) &&
		   ...
		   /*
		   * Ensure that the peer command is NULL here in case a command
		   * has completed but the R1 FP Write peer has not completed yet.
		   * If not null, it's possible that another thread will complete
		   * the peer command and should not.
		   */
		r1_cmd = NULL;
		...
		case MPI2_FUNCTION_SCSI_IO_REQUEST:  /*Fast Path IO.*/
			 ...
			 */
			if (!cmd_fusion->isRaid_1_fp_write &&
				(cmd_fusion->scmd->SCp.Status &
								MEGASAS_LOAD_BALANCE_FLAG)) {
				atomic_dec(&lbinfo->scsi_pending_cmds[cmd_fusion->pd_r1_lb]);
				cmd_fusion->scmd->SCp.Status &=
					~MEGASAS_LOAD_BALANCE_FLAG;
			} else if (cmd_fusion->isRaid_1_fp_write) {
				/* get peer command */
				...
				cmd_fusion->cmd_completed = true; /* mark it as completed */
			}
		...
		case MEGASAS_MPI2_FUNCTION_LD_IO_REQUEST: /* LD-IO Path */
			...
			* check for the raid 1/10 fast path writes
			*/
			if (r1_cmd &&  r1_cmd->isRaid_1_fp_write
				&& r1_cmd->cmd_completed) {
				/*
				 * if the peer  Raid  1/10 fast path failed,
				 * mark IO as failed to the scsi layer.
				 * over write the current status by the failed status
				 * makes sure that if any one of command fails,
				 * return fail status to scsi layer
				 */
				...
				cmd_fusion->isRaid_1_fp_write = 0;
				r1_cmd->isRaid_1_fp_write = 0;
				r1_cmd->cmd_completed = cmd_fusion->cmd_completed = false;
				megasas_return_cmd_fusion(instance, r1_cmd);
			}
			if (!cmd_fusion->isRaid_1_fp_write) {
	...
	if (megasas_wait_for_outstanding_fusion(instance, reason,
		...
		for (i = 0 ; i < instance->max_scsi_cmds; i++) {
			...
			if (instance->is_ventura) {
				...
				mpt_cmd_fusion = fusion->cmd_list[i + instance->max_fw_cmds];
				mpt_cmd_fusion->isRaid_1_fp_write = 0;
				mpt_cmd_fusion->cmd_completed = false;
				if (mpt_cmd_fusion->scmd)
					megasas_return_cmd_fusion(instance, mpt_cmd_fusion);
			}
			scmd_local = cmd_fusion->scmd;
			if (cmd_fusion->scmd) {
				scmd_local->result =
