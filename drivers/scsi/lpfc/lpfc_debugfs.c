 ...
 * Copyright (C) 2007-2017 Emulex.  All rights reserved.           *
			...
			loff_t *ppos)
{
	...
	if (phba->sli4_hba.hba_eq && io_channel) {
		...
		len += snprintf(pbuffer+len,
			LPFC_QUE_INFO_GET_BUF_SIZE-len,
			"\nHBA EQ info: EQ-STAT[max:x%x noE:x%x "
			"bs:x%x proc:x%llx]\n",
...
fcp_wq:
			...
			fcpx += phba->cfg_fcp_io_channel;
			if (fcpx < phba->cfg_fcp_max_hw_queue)
				goto fcp_wq;
		}
		...
		if ((x < phba->cfg_nvmet_mrq) &&
		    phba->sli4_hba.nvmet_cqset &&
		    phba->sli4_hba.nvmet_cqset[x]) {
			/* NVMET CQset */
			qp = phba->sli4_hba.nvmet_cqset[x];
			len += snprintf(pbuffer+len,
				LPFC_QUE_INFO_GET_BUF_SIZE-len,
				"\tNVMET CQset info: ");
			len += snprintf(pbuffer+len,
		...
		if ((x < phba->cfg_nvmet_mrq) &&
		    ...
		    phba->sli4_hba.nvmet_mrq_hdr[x] &&
		    phba->sli4_hba.nvmet_mrq_data[x]) {
			/* RQ header */
			qp = phba->sli4_hba.nvmet_mrq_hdr[x];
			...
			/* MRQ data */
			qp = phba->sli4_hba.nvmet_mrq_data[x];
			len += snprintf(pbuffer+len,
				LPFC_QUE_INFO_GET_BUF_SIZE-len,
				"\t\tDQID[%02d], "
				"QE-CNT[%04d], QE-SIZE[%04d], "
