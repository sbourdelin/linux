...
static enum cpuhp_state qedf_cpuhp_state;
 ...
 */
static int qedf_cpu_online(unsigned int cpu)
{
	struct qedf_percpu_iothread_s *iothread =
	    this_cpu_ptr(&qedf_percpu_iothreads);
	...
	if (IS_ERR(iothread->iothread)) {
		QEDF_ERR(NULL, "Failed to create I/O thread for CPU %d.\n",
		    cpu);
		return PTR_ERR(iothread->iothread);
	}
...
static int qedf_cpu_offline(unsigned int cpu)
{
	struct qedf_percpu_iothread_s *iothread;

	iothread = this_cpu_ptr(&qedf_percpu_iothreads);
...
static int __init qedf_init(void)
{
	...
	rc = cpuhp_setup_state(CPUHP_AP_ONLINE_DYN, "scsi/qedf:online",
	    qedf_cpu_online, qedf_cpu_offline);
...
err4:
	...
	cpuhp_remove_state(qedf_cpuhp_state);
