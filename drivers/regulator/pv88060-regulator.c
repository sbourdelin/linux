...
#include <linux/regulator/pv88060.h>
...
{\
	...
	.desc.ops = &pv88060_buck_ops,\
	.desc.min_uV = min, \
	.desc.uV_step = step, \
	.desc.n_voltages = ((max) - (min))/(step) + 1, \
	.desc.enable_reg = PV88060_REG_##regl_name##_CONF0, \
	.desc.enable_mask = PV88060_BUCK_EN, \
	.desc.vsel_reg = PV88060_REG_##regl_name##_CONF0, \
	.desc.vsel_mask = PV88060_VBUCK_MASK, \
...
{\
	...
	.desc.type = REGULATOR_VOLTAGE,\
	.desc.owner = THIS_MODULE,\
	.desc.ops = &pv88060_ldo_ops,\
	.desc.min_uV = min, \
	.desc.uV_step = step, \
	.desc.n_voltages = (step) ? ((max - min) / step + 1) : 1, \
	.desc.enable_reg = PV88060_REG_##regl_name##_CONF, \
	.desc.enable_mask = PV88060_LDO_EN, \
	.desc.vsel_reg = PV88060_REG_##regl_name##_CONF, \
	.desc.vsel_mask = PV88060_VLDO_MASK, \
}
...
{\
	...
	.desc.n_voltages = 1, \
	.desc.enable_reg = PV88060_REG_##regl_name##_CONF, \
	.desc.enable_mask = PV88060_SW_EN, \
}
...
#ifdef CONFIG_OF
static struct of_regulator_match pv88060_matches[] = {
	[PV88060_ID_BUCK1] = { .name = "BUCK1" },
	[PV88060_ID_LDO1] = { .name = "LDO1" },
	[PV88060_ID_LDO2] = { .name = "LDO2" },
	[PV88060_ID_LDO3] = { .name = "LDO3" },
	[PV88060_ID_LDO4] = { .name = "LDO4" },
	[PV88060_ID_LDO5] = { .name = "LDO5" },
	[PV88060_ID_LDO6] = { .name = "LDO6" },
	[PV88060_ID_LDO7] = { .name = "LDO7" },
	[PV88060_ID_SW1] = { .name = "SW1" },
	[PV88060_ID_SW2] = { .name = "SW2" },
	[PV88060_ID_SW3] = { .name = "SW3" },
	[PV88060_ID_SW4] = { .name = "SW4" },
	[PV88060_ID_SW5] = { .name = "SW5" },
	[PV88060_ID_SW6] = { .name = "SW6" },
};

static struct pv88060_pdata *pv88060_parse_regulators_dt(
		struct device *dev)
{
	struct pv88060_pdata *pdata;
	...
	n = 0;
	for (i = 0; i < ARRAY_SIZE(pv88060_matches); i++) {
		if (!pv88060_matches[i].init_data)
			continue;
		...
		pdata->init_data[n] = pv88060_matches[i].init_data;
		pdata->reg_node[n] = pv88060_matches[i].of_node;
		n++;
	}
...
static int pv88060_regulator_init(struct pv88060 *chip)
{
		...
		const struct i2c_device_id *id)
{
	...
	ret = pv88060_regulator_init(chip);
