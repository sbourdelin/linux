...
/* Global constants */
#define PESQI_BD_BUF_LEN_MAX	SZ_256
#define PESQI_BD_COUNT		SZ_256 /* max 64KB data per spi message */
					 ...
					 struct spi_transfer *transfer)
{
	...
	if (transfer->rx_buf)
		transfer->rx_dma = dma_buf;
	else
		transfer->tx_dma = dma_buf;

	return dma_buf;
}

static void pic32_sqi_unmap_transfer(struct pic32_sqi *sqi,
				     struct spi_transfer *transfer)
{
	struct device *dev = &sqi->master->dev;
	const int len = transfer->len;
	...
	if (transfer->rx_buf) {
		buf = transfer->rx_buf;
		dma_buf = transfer->rx_dma;
		dir = DMA_FROM_DEVICE;
	} else {
		buf = transfer->tx_buf;
		dma_buf = transfer->tx_dma;
		dir = DMA_TO_DEVICE;
	}
				  ...
				  struct spi_transfer *xfer)
{
	...
	/* half-duplex: select transfer direction and lane */
	if (xfer->rx_buf) {
	...
	dma_buf = pic32_sqi_map_transfer(sqi, xfer);
	if (!dma_buf)
		return -EPERM;

	/* fill BDs - one BD can handle upto 256 byte of data */
	for (offset = 0, remaining = xfer->len; remaining;) {
		/* get ring descriptor */
		rdesc = ring_desc_get(sqi);
		if (!rdesc)
			break;

		rdesc->xfer_len = min_t(u32, remaining, PESQI_BD_BUF_LEN_MAX);
		...
		/* BD BUFFER ADDRESS */
		bd->bd_addr = dma_buf + offset;

		remaining -= rdesc->xfer_len;
		offset += rdesc->xfer_len;
	}
...
xfer_out:
	/* unmap dma memory */
	list_for_each_entry_reverse(xfer, &msg->transfers, transfer_list)
		pic32_sqi_unmap_transfer(sqi, xfer);
...
static int ring_desc_ring_alloc(struct pic32_sqi *sqi)
{
	...
	/* allocate coherent memory for hardware buffer descriptors. */
	...
	sqi->ring = devm_kcalloc(&sqi->master->dev, PESQI_BD_COUNT,
				 sizeof(*rdesc), GFP_KERNEL);
	if (!sqi->ring) {
