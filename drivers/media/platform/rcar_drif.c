...
static int rcar_drif_set_format(struct rcar_drif_sdr *sdr)
{
	...
	rdrif_dbg(2, sdr, "setfmt: bitlen %u wdcnt %u num_ch %u\n",
...
static int rcar_drif_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	rdrif_dbg(1, sdr, "num hwbufs %u, hwbuf_size %u\n",
				   ...
				   struct v4l2_async_subdev *asd)
{
	...
	if (sdr->ep.asd.match.of.node != subdev->of_node) {
...
static int rcar_drif_parse_subdevs(struct rcar_drif_sdr *sdr)
{
	...
	ep = of_graph_get_next_endpoint(sdr->dev->of_node, NULL);
	if (!ep)
		return 0;
	...
	sdr->ep.asd.match.of.node = np;
	sdr->ep.asd.match_type = V4L2_ASYNC_MATCH_OF;
