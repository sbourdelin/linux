...
static int isc_clk_register(struct isc_device *isc,
		    const struct isc_clk_pdata *pdata, struct device_node *np)
{
	...
	if (num_parents < 1 || num_parents > pdata->max_num_parents)
		return -EINVAL;
...
static const struct isc_clk_pdata isc_ispck_pdata = {
	.clk_id = 0,
	.max_num_parents = 2,
};

static const struct isc_clk_pdata isc_mck_pdata = {
	.clk_id = 1,
	.max_num_parents = 3,
};

static const struct of_device_id isc_clk_ids[] = {
	{ .compatible = "atmel,sama5d2-isc-ispck", .data = &isc_ispck_pdata, },
	{ .compatible = "atmel,sama5d2-isc-mck", .data = &isc_mck_pdata, },
	{ /*sentinel*/ }
};
...
static int isc_clk_init(struct isc_device *isc)
{
	...
	for_each_child_of_node(np, childnp) {
		clk_id = of_match_node(isc_clk_ids, childnp);
		if (!clk_id)
			continue;

		ret = isc_clk_register(isc, clk_id->data, childnp);
		if (ret)
			return ret;
	}
