...
static int ov2680_write_reg(struct ov2680_dev *sensor, u16 reg, u8 val)
{
...
static int ov2680_write_reg16(struct ov2680_dev *sensor, u16 reg, u16 val)
{
	int ret;
...
static int ov2680_read_reg(struct ov2680_dev *sensor, u16 reg, u8 *val)
{
	struct i2c_client *client = sensor->i2c_client;
...
static int ov2680_read_reg16(struct ov2680_dev *sensor, u16 reg, u16 *val)
{
	u8 hi, lo;
	int ret;
...
static int ov2680_gain_set(struct ov2680_dev *sensor, bool auto_gain)
{
	...
	ret = ov2680_write_reg16(sensor, OV2680_REG_GAIN_PK, gain & 0x7ff);
...
static const struct ov2680_mode_info *
ov2680_mode_find(struct ov2680_dev *sensor, enum ov2680_frame_rate fps,
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	mode = ov2680_mode_find(sensor, sensor->current_fps, fmt->width,
				fmt->height);
	if (!mode) {
		ret = -EINVAL;
		goto unlock;
			      ...
			      struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	mode = ov2680_mode_find(sensor, sensor->current_fps, fie->width,
				fie->height);
...
static int ov2680_mode_init(struct ov2680_dev *sensor)
{
	...
	sensor->frame_interval.denominator = ov2680_framerates[OV2680_30_FPS];
	sensor->frame_interval.numerator = 1;

	sensor->current_fps = OV2680_30_FPS;
	init_mode = &ov2680_mode_data[OV2680_30_FPS][OV2680_MODE_QUXGA_800_600];
...
static int ov2680_check_id(struct ov2680_dev *sensor)
{
	...
	ret = ov2680_read_reg(sensor, OV2680_REG_CHIP_ID_HIGH, &chip_id_high);
	if (ret < 0) {
		dev_err(dev, "failed to read chip id high\n");
		return -ENODEV;
	}

	ret = ov2680_read_reg(sensor, OV2680_REG_CHIP_ID_LOW, &chip_id_low);
	if (ret < 0) {
	...
	if (chip_id_high != OV2680_CHIP_ID_HIGH ||
		...
		dev_err(dev, "chip id: 0x%02x%02x does not match expected: 0x%02x%02x\n",
			chip_id_high, chip_id_low, OV2680_CHIP_ID_HIGH,
			OV2680_CHIP_ID_LOW);
		return -ENODEV;
	}
