					...
					struct list_head *head)
{
	...
	spin_lock_irqsave(&ccidg->lock, flags);
	req = list_first_entry_or_null(head, struct usb_request, list);
	if (req)
		list_del(&req->list);
	spin_unlock_irqrestore(&ccidg->lock, flags);
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	case USB_TYPE_CLASS:
		...
		case CCIDGENERICREQ_GET_CLOCK_FREQUENCIES:
			...
			put_unaligned(ccid_class_desc.dwDefaultClock, req->buf);
		...
		case CCIDGENERICREQ_GET_DATA_RATES:
			...
			put_unaligned(ccid_class_desc.dwDataRate, req->buf);
				...
				size_t count, loff_t *pos)
{
	...
	xfer = min_t(size_t, req->actual, count);
...
{
	struct f_ccidg *ccidg = func_to_ccidg(f);
	struct f_ccidg_opts *opts = container_of(f->fi, struct f_ccidg_opts,
						func_inst);
...
static struct usb_function *ccidg_alloc(struct usb_function_instance *fi)
{
	...
	struct f_ccidg_opts *opts = container_of(fi, struct f_ccidg_opts,
						func_inst);
	int ret;
...
{
	struct f_ccidg_opts *opts = container_of(f, struct f_ccidg_opts,
						func_inst);
