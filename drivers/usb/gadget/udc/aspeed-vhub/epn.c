				   ...
				   struct ast_vhub_req *req)
{
	...
	while (ast_vhub_count_free_descs(ep) && req->last_desc < 0) {
		...
		if (chunk <= ep->epn.chunk_max) {
			 ...
			 * packes in a descriptor we can't just compare "chunk"
			 ...
			 */
			if (!req->req.zero || (chunk % ep->ep.maxpacket) != 0)
				req->last_desc = d_num;
		} else {
			      ...
			      gfp_t gfp_flags)
{
	...
	if (!u_req || !u_req->complete || !u_req->buf ||
		...
		dev_warn(&vhub->pdev->dev, "Bogus request !\n");
		return -EINVAL;
	}
	...
	EPVDBG(ep, " l=%d dma=0x%x zero=%d noshort=%d noirq=%d is_in=%d\n",
	       u_req->length, u_req->dma, u_req->zero,
			       ...
			       const struct usb_endpoint_descriptor *desc)
{
	...
	if (ep->epn.is_in && !ep->vhub->no_large_sends) {
		ep->epn.chunk_max <<= 3;
	...
	if (ep->epn.desc_mode) {
		...
		if (ep->epn.is_in && !vhub->no_large_sends)
