					    ...
					    struct usb_ss_ep_comp_descriptor *ss)
{
	...
	/*
	 * Allocate a generic EP and name it so the generic
	 * code sets the right address. We could also leave
	 * the name generic and let it auto-assign addresses
	 * it will use overlapping addresses for IN and OUT
	 * endpoints and our code isn't quite ready to deal
	 * with that yet.
	 */
	if (d->epn_count >= AST_VHUB_NUM_GEN_EPs)
		return NULL;
	// XXX Find a free slot
	addr = ++d->epn_count;
