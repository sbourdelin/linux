			       ...
			       u16 len)
{
	...
	switch (string_id) {
	case 0: /* Language IDs array */
		if (len > 4)
			len = 4;
		memcpy(ep->buf, langids, len);
		break;
	case AST_VHUB_STR_SERIAL:
		len = ascii2desc("00000000", ep->buf, len);
		break;
	case AST_VHUB_STR_PRODUCT:
		len = ascii2desc("USB Virtual Hub", ep->buf, len);
		break;
	case AST_VHUB_STR_MANUF:
		len = ascii2desc("Aspeed", ep->buf, len);
		break;
	default:
		return std_req_stall;
	}
