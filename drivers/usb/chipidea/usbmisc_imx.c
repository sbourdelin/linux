...
static enum usb_charger_type usbmisc_imx7d_charger_det(
				struct imx_usbmisc_data *data)
{
	struct imx_usbmisc *usbmisc = dev_get_drvdata(data->dev);
...
static enum usb_charger_type usbmisc_imx7d_charger_sec_det(
					struct imx_usbmisc_data *data)
{
	struct imx_usbmisc *usbmisc = dev_get_drvdata(data->dev);
	enum usb_charger_type ret = UNKNOWN_TYPE;
...
static const struct usbmisc_ops imx7d_usbmisc_ops = {
	...
	.charger_det = usbmisc_imx7d_charger_det,
	.charger_secondary_det = usbmisc_imx7d_charger_sec_det,
};
				...
				struct imx_usbmisc_data *data)
{
	...
	usbmisc = dev_get_drvdata(data->dev);
	if (!usbmisc->ops->charger_secondary_det)
		return -EINVAL;
	return usbmisc->ops->charger_secondary_det(data);
}
EXPORT_SYMBOL_GPL(imx_usbmisc_charger_secondary_det);
