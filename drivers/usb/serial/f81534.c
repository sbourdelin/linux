					...
					u8 lcr)
{
	...
	status = f81534_set_register(port, F81534_CONFIG1_REG, value);
	if (status) {
	...
	status = f81534_set_register(port, F81534_FIFO_CONTROL_REG, value);
	if (status) {
	...
	value = UART_LCR_DLAB;
	status = f81534_set_register(port, F81534_LINE_CONTROL_REG, value);
	if (status) {
	...
	value = divisor & 0xff;
	status = f81534_set_register(port, F81534_DIVISOR_LSB_REG, value);
	if (status) {
	...
	status = f81534_set_register(port, F81534_DIVISOR_MSB_REG, value);
	if (status) {
		dev_err(&port->dev, "%s: set DLAB MSB failed\n", __func__);
		return status;
	}

	status = f81534_set_register(port, F81534_LINE_CONTROL_REG, lcr);
	if (status) {
		dev_err(&port->dev, "%s: set LCR failed\n", __func__);
		return status;
	}
...
static int f81534_read_msr(struct usb_serial_port *port)
{
	...
	status = f81534_get_register(port, F81534_MODEM_STATUS_REG, &msr);
	if (status)
		return status;
