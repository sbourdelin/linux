...
/*
 * When device IP supports SS, the fifo of EP0, TX EPs, RX EPs are separated;
 * but if only supports HS, the fifo is shared for all eps, and the capability
 * registers of @EPNTXFFSZ or @EPNRXFFSZ indicate total fifo size for
 * non-ep0, and ep0 has a single 64B fifo, so other fifo starts from offset 64.
 * and the total fifo size if 64B + @EPNTXFFSZ;
 * The first 64B should be reserved for EP0, others are separated two equal
 * parts for TX or RX EPs.
 */
