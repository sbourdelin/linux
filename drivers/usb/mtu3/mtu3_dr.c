...
static void switch_port_to_host(struct ssusb_mtk *ssusb)
{
	...
	ssusb_port0_switch(ssusb, true, USB2_PORT);

	if (ssusb->otg_switch.is_u3_drd) {
		ssusb_port0_switch(ssusb, true, USB3_PORT);
...
static void switch_port_to_device(struct ssusb_mtk *ssusb)
{
	...
	ssusb_port0_switch(ssusb, false, USB2_PORT);

	if (ssusb->otg_switch.is_u3_drd) {
		ssusb_port0_switch(ssusb, false, USB3_PORT);
		check_clk = SSUSB_U3_MAC_RST_B_STS;
	}
 ...
 * we provide an interface in sysfs to switch between host and device modes
 * depending on user input.
 * It generates cable state changes by pulling up/down IDPIN and
 * notifies driver to switch mode by "extcon-usb-gpio".
 * NOTE: when use MICRO receptacle, should not provide such interface.
 */
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = sprintf(buf, "current mode: %s\n (echo device or host)\n",
