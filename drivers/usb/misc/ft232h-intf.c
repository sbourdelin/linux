 ...
 *
 * struct spi_board_info ftdi_spi_bus_info[] = {
 *	{
 *	.modalias	= "w25q32",
 *	.mode		= SPI_MODE_0,
...
static int ft232h_intf_add_mpsse_gpio(struct ft232h_intf_priv *priv)
{
	...
	dev_info(dev, "using %d MPSSE pins\n", priv->mpsse_gpio.ngpio);
	return 0;
}
...
struct spi_board_info fpga_cfg_spi_info[] = {
	{
				 ...
				 const void *plat_data)
{
	...
	if (IS_ERR(pdev)) {
		dev_info(dev, "%s: Can't create MPSSE SPI device %ld\n",
			     ...
			     const struct usb_device_id *id)
{
	...
	info = priv->info;
	if (!info) {
		dev_info(dev, "Missing device specific driver info...\n");
		return -ENODEV;
	}
