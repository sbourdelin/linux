...
/*
 * UCSI_TIMEOUT - PPM communication timeout
 *
 ...
 */
#define UCSI_TIMEOUT		1000

/*
 * UCSI_SWAP_TIMEOUT - Timeout for role swap requests
 *
...
static int ucsi_command(struct ucsi *ucsi, struct ucsi_control *ctrl)
{
	...
	return ucsi->ppm->cmd(ucsi->ppm, ctrl);
}
			    ...
			    void *data, size_t size)
{
	...
	case UCSI_ERROR:
		...
		ret = wait_for_completion_timeout(&ucsi->complete,
						msecs_to_jiffies(UCSI_TIMEOUT));
		if (!ret) {
			dev_warn(ucsi->dev, "reading error failed!\n");
			ret = -ETIMEDOUT;
			goto err_clear_flag;
		}
...
void ucsi_notify(struct ucsi *ucsi)
{
	...
	if ((cci->ack_complete || cci->cmd_complete) &&
...
static int ucsi_reset_ppm(struct ucsi *ucsi)
{
	...
	tmo = jiffies + msecs_to_jiffies(UCSI_TIMEOUT);
	...
	if (time_is_before_eq_jiffies(tmo))
		ret = -ETIMEDOUT;
...
ucsi_dr_swap(const struct typec_capability *cap, enum typec_data_role role)
{
	...
	if (!wait_for_completion_timeout(&con->complete,
					 msecs_to_jiffies(UCSI_SWAP_TIMEOUT)))
		return -ETIMEDOUT;
