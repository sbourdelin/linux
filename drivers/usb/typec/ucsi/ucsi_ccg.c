...
#define CCGX_RAB_DEVICE_MODE			0x00
#define CCGX_RAB_READ_SILICON_ID		0x2
#define CCGX_RAB_INTR_REG			0x06
#define CCGX_RAB_FW1_VERSION			0x28
#define CCGX_RAB_FW2_VERSION			0x20
#define CCGX_RAB_UCSI_CONTROL			0x39
#define CCGX_RAB_UCSI_CONTROL_START		BIT(0)
#define CCGX_RAB_UCSI_CONTROL_STOP		BIT(1)
#define CCGX_RAB_RESPONSE_REG			0x7E
...
static int ucsi_ccg_init(struct ucsi_ccg *uc)
{
	...
	status = ccg_read(uc, CCGX_RAB_DEVICE_MODE, data, sizeof(data));
	if (status < 0)
		return status;

	dev_dbg(dev, "Silicon id %2ph", data + CCGX_RAB_READ_SILICON_ID);
	dev_dbg(dev, "FW1 version %8ph\n", data + CCGX_RAB_FW1_VERSION);
	dev_dbg(dev, "FW2 version %8ph\n", data + CCGX_RAB_FW2_VERSION);

	data[0] = CCGX_RAB_UCSI_CONTROL_STOP;
	status = ccg_write(uc, CCGX_RAB_UCSI_CONTROL, data, 0x1);
	if (status < 0)
		return status;

	data[0] = CCGX_RAB_UCSI_CONTROL_START;
	status = ccg_write(uc, CCGX_RAB_UCSI_CONTROL, data, 0x1);
	if (status < 0)
		return status;
	...
	data[0] = 0xff;
	do {
		status = ccg_write(uc, CCGX_RAB_INTR_REG, data, 0x1);
		if (status < 0)
			return status;

		usleep_range(10000, 11000);

		status = ccg_read(uc, CCGX_RAB_INTR_REG, data, 0x1);
		if (status < 0)
			return status;
