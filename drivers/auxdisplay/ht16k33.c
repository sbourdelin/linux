...
#define REG_DISPLAY_SETUP_BLINK_0HZ5	(BIT(1) | BIT(2))
#define REG_DISPLAY_SETUP_BLINK_1HZ	BIT(2)
#define REG_DISPLAY_SETUP_BLINK_2HZ	BIT(1)
#define REG_DISPLAY_SETUP_ON		BIT(0)
#define REG_DISPLAY_SETUP_OFF		BIT(0)
...
#define HT16K33_FB_SIZE			(HT16K33_MATRIX_LED_MAX_COLS * 2)
...
static struct fb_var_screeninfo ht16k33_fb_var = {
	.xres = HT16K33_MATRIX_KEYPAD_MAX_ROWS,
	.yres = HT16K33_MATRIX_KEYPAD_MAX_COLS,
	.xres_virtual = HT16K33_MATRIX_KEYPAD_MAX_ROWS,
	.yres_virtual = HT16K33_MATRIX_KEYPAD_MAX_COLS,
	.bits_per_pixel = 1,
	.red = { 0, 1, 0 },
	.green = { 0, 1, 0 },
	.blue = { 0, 1, 0 },
...
static int ht16k33_bl_update_status(struct backlight_device *bl)
{
	...
	if (brightness > 0)
		...
		ht16k33_display_off(priv);
	return i2c_smbus_write_byte(priv->client, REG_BRIGHTNESS | brightness);
}
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (!fbdev->buffer) {
		dev_err(&client->dev, "Failed to allocate a free page\n");
		err = -ENOMEM;
		goto err_free_fbdev;
	}
	...
	if (!fbdev->cache) {
		dev_err(&client->dev, "Failed to allocate fb cache\n");
		err = -ENOMEM;
	...
	if (!fbdev->info) {
		dev_err(&client->dev, "Failed to allocate fbdev info\n");
		err = -ENOMEM;
		goto err_fbdev_buffer;
	}

	err = of_property_read_u32(node, "refresh-rate", &fbdev->refresh_rate);
	if (err) {
	...
	err = of_property_read_u32(node, "default-brightness", &dft_brightness);
	if (err) {
