...
static void prepopulate_bars(struct hv_pcibus_device *hbus)
{
	...
	if (hbus->low_mmio_space) {
		low_size = 1 << (63 - __builtin_clzll(hbus->low_mmio_space));
		low_base = hbus->low_mmio_res->start;
	}

	if (hbus->high_mmio_space) {
		high_size = 1 << (63 - __builtin_clzll(hbus->high_mmio_space));
			...
			const struct hv_vmbus_device_id *dev_id)
{
	...
	hbus->fwnode = irq_domain_alloc_fwnode(hbus);
	if (!hbus->fwnode) {
		ret = -ENOMEM;
		goto unmap;
	}
