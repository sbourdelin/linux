...
static int qcom_pcie_get_resources_v0(struct qcom_pcie *qcom_pcie)
{
	...
	struct device *dev = qcom_pcie->dev;
...
static int qcom_pcie_link_up(struct pcie_port *pp)
{
	...
	u16 val = readw(qcom_pcie->dbi + PCIE20_CAP + PCI_EXP_LNKSTA);
...
static int qcom_pcie_probe(struct platform_device *pdev)
{
	...
	qcom_pcie->dbi = devm_ioremap_resource(dev, res);
	if (IS_ERR(qcom_pcie->dbi))
		return PTR_ERR(qcom_pcie->dbi);
