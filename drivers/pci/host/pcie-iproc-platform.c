...
static int iproc_pcie_pltfm_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(np, "brcm,pcie-ob")) {
		...
		iproc_pcie->ob.window_size = (resource_size_t)val * SZ_1M;

		if (of_property_read_bool(np, "brcm,pcie-ob-oarr-size"))
			iproc_pcie->ob.set_oarr_size = true;

		iproc_pcie->need_ob_cfg = true;
	}
	...
	iproc_pcie->map_irq = of_irq_parse_and_map_pci;
...
{
	struct iproc_pcie *iproc_pcie = platform_get_drvdata(pdev);

	return iproc_pcie_remove(iproc_pcie);
}
