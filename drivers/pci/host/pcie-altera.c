...
static inline void cra_writel(struct altera_pcie *altera_pcie, const u32 value,
			      const u32 reg)
{
	writel_relaxed(value, altera_pcie->cra_base + reg);
}

static inline u32 cra_readl(struct altera_pcie *altera_pcie, const u32 reg)
{
				     ...
				     struct pci_bus *bus, int dev)
{
	...
	if (bus->number != altera_pcie->root_bus_nr) {
		if (!altera_pcie_link_is_up(altera_pcie))
			return false;
	}

	/* access only one slot on each root port */
	if (bus->number == altera_pcie->root_bus_nr && dev > 0)
		return false;

	 return true;
}

static int tlp_read_packet(struct altera_pcie *altera_pcie, u32 *value)
{
	int i;
	bool sop = 0;
	u32 ctrl;
...
static void tlp_write_packet(struct altera_pcie *altera_pcie, u32 *headers,
			     u32 data, bool align)
{
	struct tlp_rp_regpair_t tlp_rp_regdata;
	...
	tlp_write_tx(altera_pcie, &tlp_rp_regdata);
...
static int tlp_cfg_dword_read(struct altera_pcie *altera_pcie, u8 bus,
			      u32 devfn, int where, u8 byte_en, u32 *value)
{
	u32 headers[TLP_HDR_SIZE];

	headers[0] = TLP_CFG_DW0(altera_pcie, bus);
	headers[1] = TLP_CFG_DW1(altera_pcie, TLP_READ_TAG, byte_en);
	headers[2] = TLP_CFG_DW2(bus, devfn, where);

	tlp_write_packet(altera_pcie, headers, 0, false);

	return tlp_read_packet(altera_pcie, value);
}

static int tlp_cfg_dword_write(struct altera_pcie *altera_pcie, u8 bus,
			       u32 devfn, int where, u8 byte_en, u32 value)
{
	u32 headers[TLP_HDR_SIZE];
	int ret;
	...
	headers[0] = TLP_CFG_DW0(altera_pcie, bus);
	headers[1] = TLP_CFG_DW1(altera_pcie, TLP_WRITE_TAG, byte_en);
	headers[2] = TLP_CFG_DW2(bus, devfn, where);

	/* check alignment to Qword */
	if ((where & 0x7) == 0)
		tlp_write_packet(altera_pcie, headers, value, true);
	else
		tlp_write_packet(altera_pcie, headers, value, false);

	ret = tlp_read_packet(altera_pcie, NULL);
	 ...
	 */
	if ((bus == altera_pcie->root_bus_nr) && (where == PCI_PRIMARY_BUS))
		altera_pcie->root_bus_nr = (u8)(value);
			       ...
			       u32 *value)
{
	...
	ret = tlp_cfg_dword_read(altera_pcie, busno, devfn,
				...
				u32 value)
{
	...
	return tlp_cfg_dword_write(altera_pcie, busno, devfn,
...
{
	struct altera_pcie *altera_pcie = bus->sysdata;
...
static int altera_read_cap_word(struct altera_pcie *altera_pcie, u8 busno,
...
static void altera_wait_link_retrain(struct altera_pcie *altera_pcie)
{
	...
	for (;;) {
		altera_read_cap_word(altera_pcie, altera_pcie->root_bus_nr,
				     RP_DEVFN, PCI_EXP_LNKSTA, &reg16);
...
static void altera_pcie_retrain(struct altera_pcie *altera_pcie)
{
	 ...
	 */
	altera_read_cap_word(altera_pcie, altera_pcie->root_bus_nr, RP_DEVFN,
	...
	if ((linkstat & PCI_EXP_LNKSTA_CLS) == PCI_EXP_LNKSTA_CLS_2_5GB) {
		altera_read_cap_word(altera_pcie, altera_pcie->root_bus_nr,
				     RP_DEVFN, PCI_EXP_LNKCTL, &linkctl);
		linkctl |= PCI_EXP_LNKCTL_RL;
		altera_write_cap_word(altera_pcie, altera_pcie->root_bus_nr,
				      RP_DEVFN, PCI_EXP_LNKCTL, linkctl);

		altera_wait_link_retrain(altera_pcie);
	}
}
...
static int altera_pcie_parse_request_of_pci_ranges(struct altera_pcie *altera_pcie)
{
	int err, res_valid = 0;
	...
	err = of_pci_get_host_bridge_resources(np, 0, 0xff,
					       &altera_pcie->resources, NULL);
	if (err)
		return err;

	err = devm_request_pci_bus_resources(dev, &altera_pcie->resources);
	if (err)
		goto out_release_res;

	resource_list_for_each_entry(win, &altera_pcie->resources) {
...
out_release_res:
	pci_free_resource_list(&altera_pcie->resources);
	return err;
}

static int altera_pcie_init_irq_domain(struct altera_pcie *altera_pcie)
{
	struct device *dev = &altera_pcie->pdev->dev;
	struct device_node *node = dev->of_node;

	/* Setup INTx */
	altera_pcie->irq_domain = irq_domain_add_linear(node, INTX_NUM + 1,
					&intx_domain_ops, altera_pcie);
	if (!altera_pcie->irq_domain) {
...
static void altera_pcie_host_init(struct altera_pcie *altera_pcie)
{
	altera_pcie_retrain(altera_pcie);
}
...
static int altera_pcie_probe(struct platform_device *pdev)
{
	...
	INIT_LIST_HEAD(&altera_pcie->resources);
	...
	platform_set_drvdata(pdev, altera_pcie);
	return ret;
}
