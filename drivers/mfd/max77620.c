			       ...
			       struct device_node *fps_np)
{
	...
	ret = of_property_read_u32(fps_np, "maxim,fps-control", &param_val);
	if (!ret) {
		if (param_val > 2) {
		...
		config |= (param_val & 0x3) << MAX77620_FPS_EN_SRC_SHIFT;
		if (param_val == 2) {
	...
	if (!chip->sleep_enable)
		chip->sleep_enable =
			of_property_read_bool(fps_np, "maxim,enable-sleep");

	if (!chip->enable_global_lpm)
		chip->enable_global_lpm =
			of_property_read_bool(fps_np,
