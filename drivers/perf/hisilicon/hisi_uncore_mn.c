...
{
	const unsigned long *used_mask = mn_pmu->pmu_events.used_mask;
	u32 num_counters = mn_pmu->num_counters;
	int event_idx;
	...
	set_bit(event_idx, mn_pmu->pmu_events.used_mask);
			     ...
			     struct hisi_djtag_client *client)
{
	...
	/*
	 * Use poll method to avoid counter overflow when
			     ...
			     struct hisi_djtag_client *client)
{
	...
	mn_pmu->name = devm_kasprintf(dev, GFP_KERNEL, "hisi_mn_%d",
				      mn_pmu->scl_id);
