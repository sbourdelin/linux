...
static struct i3c_bus *dev_to_i3cbus(struct device *dev)
{
	...
	return container_of(dev, struct i3c_bus, dev);
}
...
static struct i3c_dev_desc *dev_to_i3cdesc(struct device *dev)
{
	...
	bus = container_of(dev, struct i3c_bus, dev);

	return bus->master->this;
}
...
struct i3c_bus *i3c_bus_create(struct i3c_master_controller *master)
{
	...
	i3cbus->master = master;
	init_rwsem(&i3cbus->lock);
	INIT_LIST_HEAD(&i3cbus->devs.i2c);
	INIT_LIST_HEAD(&i3cbus->devs.i3c);
	i3c_bus_init_addrslots(i3cbus);
	i3cbus->mode = I3C_BUS_MODE_PURE;
	i3cbus->dev.parent = master->parent;
	i3cbus->dev.of_node = of_node_get(master->parent->of_node);
