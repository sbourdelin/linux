			    ...
			    char *start, char *stop)
{
	...
	/* parsing HT_CAP_IE */
	p = rtw_get_ie(&pnetwork->network.ie[12], _HT_CAPABILITY_IE_, &ht_ielen, pnetwork->network.ie_length-12);
	...
	start = iwe_stream_add_event(info, start, stop, &iwe, IW_EV_CHAR_LEN);
	  ...
	  /* Add mode */
	iwe.cmd = SIOCGIWMODE;
	memcpy(&le_tmp, rtw_get_capability_from_ie(pnetwork->network.ie), 2);
	...
	/* parsing WPA/WPA2 IE */
	{
		...
		rtw_get_sec_ie(pnetwork->network.ie, pnetwork->network.ie_length, rsn_ie, &rsn_len, wpa_ie, &wpa_len);
	...
	{/* parsing WPS IE */
		...
		u8 *wpsie_ptr = NULL;
		uint wps_ielen = 0;
		u8 *ie_ptr = pnetwork->network.ie + _FIXED_IE_LENGTH_;
			     ...
			     union iwreq_data *wrqu, char *extra)
{
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED|WIFI_ADHOC_MASTER_STATE) == true) {
		/* parsing HT_CAP_IE */
		p = rtw_get_ie(&pcur_bss->ie[12], _HT_CAPABILITY_IE_, &ht_ielen, pcur_bss->ie_length-12);
		if (p && ht_ielen > 0)
			ht_cap = true;
