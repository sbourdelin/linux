		...
		if (check_fwstate(pmlmepriv, WIFI_STATION_STATE | WIFI_MP_STATE)) { /* sta mode */
			...
			if (psta == NULL) {
				...
				if (param->u.crypt.set_tx == 1) { /* pairwise key */
					memcpy(psta->dot118021x_UncstKey.skey,  param->u.crypt.key, min(param->u.crypt.key_len, (u16)16));
					...
					memcpy(padapter->securitypriv.dot118021XGrpKey[param->u.crypt.idx].skey, param->u.crypt.key, min(param->u.crypt.key_len, (u16)16));
			      ...
			      union iwreq_data *wrqu, char *extra)
{
	...
	if (wrqu->essid.flags && wrqu->essid.length) {
		len = min(wrqu->essid.length, (__u16)IW_ESSID_MAX_SIZE);
		...
		if (param->u.crypt.set_tx == 1) {
			if (strcmp(param->u.crypt.alg, "WEP") == 0) {
				...
				memcpy(psecuritypriv->dot118021XGrpKey[param->u.crypt.idx].skey,
					    param->u.crypt.key, min(param->u.crypt.key_len, (u16)16));
