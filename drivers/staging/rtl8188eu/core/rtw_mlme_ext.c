...
static void issue_beacon(struct adapter *padapter, int timeout_ms)
{
	...
	*(fctrl) = 0;
	...
	if ((pmlmeinfo->state&0x03) == WIFI_FW_AP_STATE) {
		...
		memcpy(pframe, cur_network->ie, cur_network->ie_length);
	...
	memcpy(pframe, (unsigned char *)(rtw_get_beacon_interval_from_ie(cur_network->ie)), 2);
	...
	memcpy(pframe, (unsigned char *)(rtw_get_capability_from_ie(cur_network->ie)), 2);
...
#if defined(CONFIG_88EU_AP_MODE)
	if ((pmlmeinfo->state&0x03) == WIFI_FW_AP_STATE) {
		pwps_ie = rtw_get_wps_ie(cur_network->ie+_FIXED_IE_LENGTH_, cur_network->ie_length-_FIXED_IE_LENGTH_, NULL, &wps_ielen);
		...
		if ((pmlmepriv->wps_probe_resp_ie != NULL) && pwps_ie && (wps_ielen > 0)) {
			...
			wps_offset = (uint)(pwps_ie - cur_network->ie);

			premainder_ie = pwps_ie + wps_ielen;

			remainder_ielen = cur_network->ie_length - wps_offset - wps_ielen;

			memcpy(pframe, cur_network->ie, wps_offset);
			pframe += wps_offset;
...
static void issue_assocreq(struct adapter *padapter)
{
	...
	*(fctrl) = 0;
	...
	memcpy(pframe, rtw_get_capability_from_ie(pmlmeinfo->network.ie), 2);
	...
	/* RSN */
	p = rtw_get_ie((pmlmeinfo->network.ie + sizeof(struct ndis_802_11_fixed_ie)), _RSN_IE_2_, &ie_len, (pmlmeinfo->network.ie_length - sizeof(struct ndis_802_11_fixed_ie)));
	if (p != NULL)
		pframe = rtw_set_ie(pframe, _RSN_IE_2_, ie_len, (p + 2), &(pattrib->pktlen));

	/* HT caps */
	if (padapter->mlmepriv.htpriv.ht_option) {
		p = rtw_get_ie((pmlmeinfo->network.ie + sizeof(struct ndis_802_11_fixed_ie)), _HT_CAPABILITY_IE_, &ie_len, (pmlmeinfo->network.ie_length - sizeof(struct ndis_802_11_fixed_ie)));
	...
	for (i = sizeof(struct ndis_802_11_fixed_ie); i < pmlmeinfo->network.ie_length; i += (pIE->Length + 2)) {
		pIE = (struct ndis_802_11_var_ie *)(pmlmeinfo->network.ie + i);
	...
	*(fctrl) = 0;
	...
	if (pmlmepriv->num_sta_no_ht > 0) {
		...
		while (phead != plist) {
			...
			p = rtw_get_ie(pbss_network->ie + _FIXED_IE_LENGTH_, _HT_CAPABILITY_IE_, &len, pbss_network->ie_length - _FIXED_IE_LENGTH_);
			   ...
			   struct wlan_bssid_ex *bssid)
{
	...
	bssid->ie_length = len;
	memcpy(bssid->ie, (pframe + sizeof(struct ieee80211_hdr_3addr)), bssid->ie_length);
	...
	/*  checking SSID */
	p = rtw_get_ie(bssid->ie + ie_offset, _SSID_IE_, &len, bssid->ie_length - ie_offset);
	if (p == NULL) {
	...
	/* checking rate info... */
	i = 0;
	p = rtw_get_ie(bssid->ie + ie_offset, _SUPPORTEDRATES_IE_, &len, bssid->ie_length - ie_offset);
	if (p != NULL) {
	...
	p = rtw_get_ie(bssid->ie + ie_offset, _EXT_SUPPORTEDRATES_IE_, &len, bssid->ie_length - ie_offset);
	if (p != NULL) {
	...
	p = rtw_get_ie(bssid->ie + ie_offset, _DSSET_IE_, &len, bssid->ie_length - ie_offset);
	...
	if (p) {
		...
		p = rtw_get_ie(bssid->ie + ie_offset, _HT_ADD_INFO_IE_, &len, bssid->ie_length - ie_offset);
		if (p) {
	...
	bssid->Configuration.BeaconPeriod =
		get_unaligned_le16(rtw_get_beacon_interval_from_ie(bssid->ie));
	...
	if ((pregistrypriv->wifi_spec == 1) && (!pmlmeinfo->bwmode_updated)) {
		...
		p = rtw_get_ie(bssid->ie + ie_offset, _HT_CAPABILITY_IE_, &len, bssid->ie_length - ie_offset);
		if (p && len > 0) {
...
static void process_80211d(struct adapter *padapter, struct wlan_bssid_ex *bssid)
{
	...
	if (pregistrypriv->enable80211d &&
		...
		ie = rtw_get_ie(bssid->ie + _FIXED_IE_LENGTH_, _COUNTRY_IE_, &len, bssid->ie_length - _FIXED_IE_LENGTH_);
		if (!ie)
			return;
		if (len < 6)
			return;
...
u8 createbss_hdl(struct adapter *padapter, u8 *pbuf)
{
	...
	if (pparm->InfrastructureMode == Ndis802_11IBSS) {
		...
		memcpy(pnetwork->ie, ((struct wlan_bssid_ex *)pbuf)->ie, pnetwork->ie_length);
...
u8 set_tx_beacon_cmd(struct adapter *padapter)
{
	...
	len_diff = update_hidden_ssid(ptxBeacon_parm->ie+_BEACON_IE_OFFSET_,
