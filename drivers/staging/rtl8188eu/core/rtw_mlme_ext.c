...
static void issue_beacon(struct adapter *padapter, int timeout_ms)
{
	...
	*(fctrl) = 0;
	...
	pframe = rtw_set_ie(pframe, _SUPPORTEDRATES_IE_, min(rate_len, (unsigned int)8), cur_network->SupportedRates, &pattrib->pktlen);
			       ...
			       struct recv_frame *precv_frame)
{
	...
	if (wpa_ie == NULL) {
		...
		if (elems.wps_ie) {
			...
			copy_len = min(wpa_ie_len + 2, (int)sizeof(pstat->wpa_ie));
		}
		if (copy_len > 0)
