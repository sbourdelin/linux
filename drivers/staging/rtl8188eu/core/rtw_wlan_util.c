...
u16 get_beacon_interval(struct wlan_bssid_ex *bss)
{
	...
	memcpy((unsigned char *)&val, rtw_get_beacon_interval_from_ie(bss->ie), 2);
...
int rtw_check_bcn_info(struct adapter  *Adapter, u8 *pframe, u32 packet_len)
{
	...
	/* parsing HT_CAP_IE */
	p = rtw_get_ie(bssid->ie + _FIXED_IE_LENGTH_, _HT_CAPABILITY_IE_, &len, bssid->ie_length - _FIXED_IE_LENGTH_);
	if (p && len > 0) {
	...
	/* parsing HT_INFO_IE */
	p = rtw_get_ie(bssid->ie + _FIXED_IE_LENGTH_, _HT_ADD_INFO_IE_, &len, bssid->ie_length - _FIXED_IE_LENGTH_);
	if (p && len > 0) {
	...
	p = rtw_get_ie(bssid->ie + _FIXED_IE_LENGTH_, _DSSET_IE_, &len, bssid->ie_length - _FIXED_IE_LENGTH_);
	if (p) {
			bcn_channel = *(p + 2);
	...
	/* checking SSID */
	ssid_len = 0;
	p = rtw_get_ie(bssid->ie + _FIXED_IE_LENGTH_, _SSID_IE_, &len, bssid->ie_length - _FIXED_IE_LENGTH_);
	if (p) {
		ssid_len = *(p + 1);
	...
	rtw_get_sec_ie(bssid->ie, bssid->ie_length, NULL, &rsn_len, NULL, &wpa_len);
	...
	if (encryp_protocol == ENCRYP_PROTOCOL_WPA || encryp_protocol == ENCRYP_PROTOCOL_WPA2) {
		pbuf = rtw_get_wpa_ie(&bssid->ie[12], &wpa_ielen, bssid->ie_length-12);
		if (pbuf && (wpa_ielen > 0)) {
			...
			pbuf = rtw_get_wpa2_ie(&bssid->ie[12], &wpa_ielen, bssid->ie_length-12);
...
unsigned int should_forbid_n_rate(struct adapter *padapter)
{
	...
	if (rtw_get_capability((struct wlan_bssid_ex *)cur_network) & WLAN_CAPABILITY_PRIVACY) {
		for (i = sizeof(struct ndis_802_11_fixed_ie); i < cur_network->ie_length;) {
			pIE = (struct ndis_802_11_var_ie *)(cur_network->ie + i);
