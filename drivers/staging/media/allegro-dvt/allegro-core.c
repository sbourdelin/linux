...
#define ALLEGRO_WIDTH_DEFAULT 144
#define ALLEGRO_WIDTH_MAX 1920
#define ALLEGRO_HEIGHT_MIN 64
#define ALLEGRO_HEIGHT_DEFAULT 144
...
/*
 * Slice types reported in the mcu_msg_encode_one_frm_response message.
 */
...
bool poison_capture_buffers = false;
...
enum mcu_msg_type {
	...
	MCU_MSG_TYPE_ENCODE_ONE_FRM = 0x0007,
...
static const char *msg_type_name(enum mcu_msg_type type)
{
	...
	case MCU_MSG_TYPE_ENCODE_ONE_FRM:
		return "ENCODE_ONE_FRM";
...
struct mcu_msg_encode_one_frm_response {
	struct mcu_msg_header header;
	u32 channel_id;
	u64 stream_id;		/* see mcu_msg_put_stream_buffer */
	u64 user_param;		/* see mcu_msg_encode_one_frm */
	u64 src_handle;		/* see mcu_msg_encode_one_frm */
	u16 skip;
	u16 is_ref;
					    ...
					    struct allegro_channel *channel)
{
	...
	msg.rate_control_mode = 0x00000001;	/* AL_RC_CBR */
	msg.initial_rem_delay = 90000;	/* 90000 -> 1s */
	msg.cpb_size = 90000;	/* 90000 -> 1s */
	msg.framerate = 25;
	msg.clk_ratio = 1000;
	msg.target_bitrate = 700000;	/* 700000 -> 700 kbit/s */
	msg.max_bitrate = 700000;	/* 700000 -> 700 kbit/s */
	msg.initial_qp = 25;
	msg.min_qp = 10;
	msg.max_qp = 51;
	msg.ip_delta = -1;
					      ...
					      struct allegro_buffer buffer)
{
	 ...
	 * the used stream buffer from the ENCODE_ONE_FRM response.
	 */
	int stream_id = 0;
...
static void allegro_mcu_send_encode_one_frm(struct allegro_dev *dev,
					    struct allegro_channel *channel,
					    dma_addr_t src_y, dma_addr_t src_uv)
{
	struct mcu_msg_encode_one_frm msg;

	msg.header.type = MCU_MSG_TYPE_ENCODE_ONE_FRM;
	/*
...
{
	struct allegro_dev *dev = channel->dev;
	struct nal_h264_sps *sps = kzalloc(sizeof(*sps), GFP_KERNEL);
	ssize_t size;

	v4l2_dbg(1, debug, &dev->v4l2_dev, "write sps nal unit\n");
	...
	sps->vui.vcl_hrd_parameters.bit_rate_value_minus1[0] = 10936;
	sps->vui.vcl_hrd_parameters.cpb_size_value_minus1[0] = 21874;
...
{
	struct allegro_dev *dev = channel->dev;
	struct nal_h264_pps *pps = kzalloc(sizeof(*pps), GFP_KERNEL);
	ssize_t size;

	v4l2_dbg(1, debug, &dev->v4l2_dev, "write pps nal unit\n");
...
static void allegro_finish_frame(struct allegro_channel *channel,
				 struct mcu_msg_encode_one_frm_response *msg)
{
	struct allegro_dev *dev = channel->dev;
	...
	v4l2_dbg(1, debug, &dev->v4l2_dev,
			"write %ld bytes filler nal unit\n", free);
	...
	v4l2_dbg(1, debug, &dev->v4l2_dev,
		"channel %d: encoded frame (%s%s, size = %d)\n",
		channel->mcu_channel_id,
		msg->is_idr ? "IDR, " : "",
...
static void allegro_receive_message(struct allegro_dev *dev)
{
	struct mcu_msg_header *header;
	ssize_t size;

	header = kmalloc(sizeof(struct mcu_msg_encode_one_frm_response),
			 GFP_KERNEL);
	if (!header)
		return;

	size = allegro_mbox_read(dev, &dev->mbox_status, header,
				 sizeof(struct mcu_msg_encode_one_frm_response));
	...
	if (header->type == MCU_MSG_TYPE_INIT) {
		complete(&dev->init_complete);
	} else if (header->type == MCU_MSG_TYPE_CREATE_CHANNEL) {
		struct mcu_msg_create_channel_response *msg;
		struct allegro_channel *channel;

		msg = (struct mcu_msg_create_channel_response *)header;

		channel = allegro_find_channel_by_user_id(dev, msg->user_id);
		if (IS_ERR(channel)) {
			v4l2_err(&dev->v4l2_dev,
				 "received %s for unknown user %d\n",
				 msg_type_name(msg->header.type), msg->user_id);
			goto out;
		}

		channel->mcu_channel_id = msg->channel_id;
		v4l2_dbg(2, debug, &dev->v4l2_dev,
			"user %d: vcu created channel %d\n",
			channel->user_id, channel->mcu_channel_id);

		v4l2_dbg(1, debug, &dev->v4l2_dev,
			"channel %d: vcu requested intermediate buffers: %d x %d bytes\n",
			channel->mcu_channel_id, msg->int_buffers_count,
			msg->int_buffers_size);
		allocate_intermediate_buffers(channel, msg->int_buffers_count,
					      msg->int_buffers_size);
		allegro_mcu_push_buffer_intermediate(channel);

		v4l2_dbg(1, debug, &dev->v4l2_dev,
			"channel %d: vcu requested reference buffers: %d x %d bytes\n",
			channel->mcu_channel_id, msg->rec_buffers_count,
			msg->rec_buffers_size);
		allocate_reference_buffers(channel, msg->rec_buffers_count,
					   msg->rec_buffers_size);
		allegro_mcu_push_buffer_reference(channel);

		complete(&channel->completion);
	} else if (header->type == MCU_MSG_TYPE_DESTROY_CHANNEL) {
		struct mcu_msg_destroy_channel_response *msg;
		struct allegro_channel *channel;

		msg = (struct mcu_msg_destroy_channel_response *)header;

		channel =
		    allegro_find_channel_by_channel_id(dev, msg->channel_id);
		if (IS_ERR(channel)) {
			v4l2_err(&dev->v4l2_dev,
		...
		v4l2_dbg(2, debug, &dev->v4l2_dev,
			...
			channel->user_id, channel->mcu_channel_id);
		/* FIXME 0 is a valid channel id */
		channel->mcu_channel_id = 0;

		complete(&channel->completion);
	} else if (header->type == MCU_MSG_TYPE_ENCODE_ONE_FRM) {
		struct mcu_msg_encode_one_frm_response *msg;
		struct allegro_channel *channel;

		if (header->length != sizeof(*msg) - sizeof(*header)) {
			v4l2_err(&dev->v4l2_dev,
				 "message %s: wrong size: expected %d, actual %ld\n",
				 msg_type_name(header->type), header->length,
				 sizeof(*msg) - sizeof(*header));
			goto out;
		}

		msg = (struct mcu_msg_encode_one_frm_response *)header;
...
irqreturn_t allegro_hardirq(int irq, void *data)
{
	...
	/*
	 * TODO The downstream driver suggests to wait for AL5_MCU_IRQ_STA ==
	 * 0, but does not actually implement it.
	 */
...
static void allegro_mcu_start(struct allegro_dev *dev)
{
	regmap_write(dev->regmap, AL5_MCU_WAKEUP, BIT(0));

	/*
	 * TODO The register documentation suggests that AL5_MCU_WAKEUP should
	 * be reset to 0 after AL5_MCU_STA_SLEEP get back to 0.
	 */
}
...
static int allegro_mcu_wait_for_sleep(struct allegro_dev *dev)
{
	...
	/* Downstream driver waits for 100 ms. */
...
static void allegro_mcu_stop(struct allegro_dev *dev)
{
	int err;

	regmap_write(dev->regmap, AL5_MCU_WAKEUP, 0);

	/*
	 * Write the MicroBlaze "mbar 16" instruction (binary 0xba020004) into
	 * the MCU interrupt handler. 16 sets the msb in the IMM value of the
	 * mbar instruction and tells the MicroBlaze to finish all outstanding
	 * accesses and enter sleep mode. The pipeline is halted and the
	 * MicroBlaze waits for the WAKEUP signal.
	 */
	regmap_write(dev->regmap, 0x10, 0xba020004);
	allegro_mcu_interrupt(dev);

	err = allegro_mcu_wait_for_sleep(dev);
	if (err == -ETIMEDOUT) {
		v4l2_err(&dev->v4l2_dev,
			 "MCU failed to enter sleep mode\n");
		return;
	}
}

static void allegro_mcu_reset(struct allegro_dev *dev)
{
	regmap_write(dev->regmap, AL5_MCU_WAKEUP, 0);

	regmap_write(dev->regmap, AL5_MCU_RESET_MODE, AL5_MCU_RESET_MODE_SLEEP);
	regmap_write(dev->regmap, AL5_MCU_RESET, AL5_MCU_RESET_SOFT);
}
...
static int allegro_create_channel(struct allegro_channel *channel)
{
	...
	v4l2_dbg(1, debug, &dev->v4l2_dev,
		 "channel %d: ready to accept buffers\n",
		 channel->mcu_channel_id);
...
static void allegro_set_default_params(struct allegro_channel *channel)
{
	...
	channel->level = V4L2_MPEG_VIDEO_H264_LEVEL_2_0;
...
static void allegro_buf_queue(struct vb2_buffer *vb)
{
	...
	if (vq->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
		...
		v4l2_dbg(2, debug, &dev->v4l2_dev,
			 "queuing stream buffer: paddr: 0x%08llx, size: 0x%08lx\n",
			 al_buf.paddr, al_buf.size);
		allegro_mcu_send_put_stream_buffer(dev, channel, al_buf);
	} else {
		v4l2_dbg(2, debug, &dev->v4l2_dev, "queuing source buffer\n");
	}
				   ...
				   struct v4l2_format *f)
{
	...
	f->fmt.pix.width = ALLEGRO_WIDTH_DEFAULT;
	f->fmt.pix.height = ALLEGRO_HEIGHT_DEFAULT;

	f->fmt.pix.pixelformat = V4L2_PIX_FMT_H264;
	f->fmt.pix.flags = V4L2_FMT_FLAG_COMPRESSED;
	f->fmt.pix.bytesperline = ALLEGRO_WIDTH_DEFAULT;
	f->fmt.pix.sizeimage =
...
static void allegro_device_run(void *priv)
{
	...
	allegro_mcu_send_encode_one_frm(dev, channel, src_y, src_uv);
}
...
static void allegro_fw_callback(const struct firmware *fw, void *context)
{
	...
	/* TODO The downstream driver stops and resets the mcu, too. */
	allegro_mcu_stop(dev);
...
{
	struct allegro_dev *dev =
	    container_of(ctrl->handler, struct allegro_dev, ctrl_handler);
...
static int allegro_probe(struct platform_device *pdev)
{
	...
	v4l2_ctrl_handler_init(&dev->ctrl_handler, 0);
	v4l2_ctrl_new_std_menu(&dev->ctrl_handler, &allegro_ctrl_ops,
			       V4L2_CID_MPEG_VIDEO_H264_PROFILE,
			       V4L2_MPEG_VIDEO_H264_PROFILE_BASELINE, 0x0,
			       V4L2_MPEG_VIDEO_H264_PROFILE_BASELINE);
	v4l2_ctrl_new_std_menu(&dev->ctrl_handler, &allegro_ctrl_ops,
			       V4L2_CID_MPEG_VIDEO_H264_LEVEL,
			       V4L2_MPEG_VIDEO_H264_LEVEL_2_0, 0x0,
