...
static void npcmgpio_dbg_show(struct seq_file *s, struct gpio_chip *chip)
{
	...
	seq_printf(s, "DIN :%.8x DOUT:%.8x IE  :%.8x OE	 :%.8x\n",
		   ioread32(base + NPCM7XX_GP_N_DIN),
		   ioread32(base + NPCM7XX_GP_N_DOUT),
		   ioread32(base + NPCM7XX_GP_N_IEM),
		   ioread32(base + NPCM7XX_GP_N_OE));
	seq_printf(s, "PU  :%.8x PD  :%.8x DB  :%.8x POL :%.8x\n",
		   ioread32(base + NPCM7XX_GP_N_PU),
		   ioread32(base + NPCM7XX_GP_N_PD),
		   ioread32(base + NPCM7XX_GP_N_DBNC),
		   ioread32(base + NPCM7XX_GP_N_POL));
	seq_printf(s, "ETYP:%.8x EVBE:%.8x EVEN:%.8x EVST:%.8x\n",
		   ioread32(base + NPCM7XX_GP_N_EVTYP),
		   ioread32(base + NPCM7XX_GP_N_EVBE),
		   ioread32(base + NPCM7XX_GP_N_EVEN),
		   ioread32(base + NPCM7XX_GP_N_EVST));
	seq_printf(s, "OTYP:%.8x OSRC:%.8x ODSC:%.8x\n",
		   ioread32(base + NPCM7XX_GP_N_OTYP),
		   ioread32(base + NPCM7XX_GP_N_OSRC),
		   ioread32(base + NPCM7XX_GP_N_ODSC));
	seq_printf(s, "OBL0:%.8x OBL1:%.8x OBL2:%.8x OBL3:%.8x\n",
		   ioread32(base + NPCM7XX_GP_N_OBL0),
		   ioread32(base + NPCM7XX_GP_N_OBL1),
		   ioread32(base + NPCM7XX_GP_N_OBL2),
		   ioread32(base + NPCM7XX_GP_N_OBL3));
	seq_printf(s, "SLCK:%.8x MLCK:%.8x\n",
		   ioread32(base + NPCM7XX_GP_N_SPLCK),
		   ioread32(base + NPCM7XX_GP_N_MPLCK));
}
				     ...
				     unsigned int offset, int value)
{
	...
	gpio_bitop(bank, op_setbit, offset, value ? NPCM7XX_GP_N_DOS :
		   NPCM7XX_GP_N_DOC);
...
static int npcmgpio_get_value(struct gpio_chip *chip, unsigned int offset)
{
	...
	return gpio_bitop(bank, op_getbit, offset, dir == GPIOF_DIR_OUT ?
			  NPCM7XX_GP_N_DOUT : NPCM7XX_GP_N_DIN);
}
...
static int npcmgpio_set_irq_type(struct irq_data *d, unsigned int type)
{
	...
	case IRQ_TYPE_EDGE_RISING:
		...
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_EVBE);
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_POL);
		break;
	case IRQ_TYPE_EDGE_FALLING:
		dev_dbg(d->chip->parent_device, "edge.falling\n");
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_EVBE);
		gpio_bitop(bank, op_setbit, gpio, NPCM7XX_GP_N_POL);
		break;
	case IRQ_TYPE_EDGE_BOTH:
		dev_dbg(d->chip->parent_device, "edge.both\n");
		gpio_bitop(bank, op_setbit, gpio, NPCM7XX_GP_N_EVBE);
		break;
	...
	if (type & (IRQ_TYPE_LEVEL_HIGH | IRQ_TYPE_LEVEL_LOW)) {
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_EVTYP);
		irq_set_handler_locked(d, handle_level_irq);
	} else if (type & (IRQ_TYPE_EDGE_BOTH | IRQ_TYPE_EDGE_RISING
			   | IRQ_TYPE_EDGE_FALLING)) {
		gpio_bitop(bank, op_setbit, gpio, NPCM7XX_GP_N_EVTYP);
...
static void npcmgpio_irq_ack(struct irq_data *d)
{
	...
	gpio_bitop(bank, op_set, gpio, NPCM7XX_GP_N_EVST);
}
...
static void npcmgpio_irq_mask(struct irq_data *d)
{
	...
	gpio_bitop(bank, op_set, gpio, NPCM7XX_GP_N_EVENC);
}
...
static void npcmgpio_irq_unmask(struct irq_data *d)
{
	...
	gpio_bitop(bank, op_set, gpio, NPCM7XX_GP_N_EVENS);
}
				 ...
				 struct regmap *gcr_regmap, unsigned int pin)
{
	...
	if (pincfg[pin].flag & SLEW)
		return gpio_bitop(bank, op_getbit, gpio, NPCM7XX_GP_N_OSRC);
				 ...
				 int arg)
{
	...
	if (pincfg[pin].flag & SLEW) {
		switch (arg) {
		case 0:
			gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_OSRC);
			return 0;
		case 1:
			gpio_bitop(bank, op_setbit, gpio, NPCM7XX_GP_N_OSRC);
			return 0;
		default:
			return -EINVAL;
		}
				      ...
				      unsigned int pin)
{
	...
	if (flg & DRIVE_STRENGTH_MASK) {
		...
		val = gpio_bitop(bank, op_getbit, gpio, NPCM7XX_GP_N_ODSC);
				      ...
				      unsigned int pin, int nval)
{
	...
	if (DSLO(v) == nval) {
		...
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_ODSC);
		return 0;
	} else if (DSHI(v) == nval) {
		dev_dbg(bank->gc.parent,
			"setting pin %d to high strength [%d]\n", pin, nval);
		gpio_bitop(bank, op_setbit, gpio, NPCM7XX_GP_N_ODSC);
		return 0;
	}
...
/* Set GPIO direction */
static int npcm7xx_gpio_set_direction(struct pinctrl_dev *pctldev,
				      struct pinctrl_gpio_range *range,
				      unsigned int offset, bool input)
{
	...
	dev_dbg(bank->gc.parent, "GPIO Set Direction: %d = %d\n", offset,
		input);
	if (input) {
		gpio_bitop(bank, op_set, gpio, NPCM7XX_GP_N_OEC);
		gpio_bitop(bank, op_setbit, gpio, NPCM7XX_GP_N_IEM);
	} else {
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_IEM);
		gpio_bitop(bank, op_set, gpio, NPCM7XX_GP_N_OES);
	}
...
static struct pinmux_ops npcm7xx_pinmux_ops = {
	...
	.gpio_set_direction = npcm7xx_gpio_set_direction,
};
			      ...
			      unsigned long *config)
{
	...
	case PIN_CONFIG_BIAS_PULL_DOWN:
		pu = gpio_bitop(bank, op_getbit, gpio, NPCM7XX_GP_N_PU);
		pd = gpio_bitop(bank, op_getbit, gpio, NPCM7XX_GP_N_PD);
	...
	case PIN_CONFIG_INPUT_ENABLE:
		ie = gpio_bitop(bank, op_getbit, gpio, NPCM7XX_GP_N_IEM);
		oe = gpio_bitop(bank, op_getbit, gpio, NPCM7XX_GP_N_OE);
	...
	case PIN_CONFIG_DRIVE_PUSH_PULL:
		rc = !gpio_bitop(bank, op_getbit, gpio, NPCM7XX_GP_N_OTYP);
		break;
	case PIN_CONFIG_DRIVE_OPEN_DRAIN:
		rc = gpio_bitop(bank, op_getbit, gpio, NPCM7XX_GP_N_OTYP);
		break;
	case PIN_CONFIG_INPUT_DEBOUNCE:
		rc = gpio_bitop(bank, op_getbit, gpio, NPCM7XX_GP_N_DBNC);
		break;
				  ...
				  unsigned int pin, unsigned long config)
{
	...
	switch (param) {
	case PIN_CONFIG_BIAS_DISABLE:
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_PU);
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_PD);
		break;
	case PIN_CONFIG_BIAS_PULL_DOWN:
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_PU);
		gpio_bitop(bank, op_setbit, gpio, NPCM7XX_GP_N_PD);
		break;
	case PIN_CONFIG_BIAS_PULL_UP:
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_PD);
		gpio_bitop(bank, op_setbit, gpio, NPCM7XX_GP_N_PU);
		break;
	case PIN_CONFIG_INPUT_ENABLE:
		if (arg)
			gpio_bitop(bank, op_set, gpio, NPCM7XX_GP_N_OEC);
		gpio_bitop(bank, arg ? op_setbit : op_clrbit, gpio,
			   NPCM7XX_GP_N_IEM);
		break;
	case PIN_CONFIG_OUTPUT:
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_IEM);
		gpio_bitop(bank, op_set, gpio, arg ? NPCM7XX_GP_N_DOS :
			   NPCM7XX_GP_N_DOC);
		gpio_bitop(bank, op_set, gpio, NPCM7XX_GP_N_OES);
		break;
	case PIN_CONFIG_DRIVE_PUSH_PULL:
		gpio_bitop(bank, op_clrbit, gpio, NPCM7XX_GP_N_OTYP);
		break;
	case PIN_CONFIG_DRIVE_OPEN_DRAIN:
		gpio_bitop(bank, op_setbit, gpio, NPCM7XX_GP_N_OTYP);
		break;
	case PIN_CONFIG_INPUT_DEBOUNCE:
		gpio_bitop(bank, op_setbit, gpio, NPCM7XX_GP_N_DBNC);
		break;
	case PIN_CONFIG_SLEW_RATE:
...
static struct gpio_chip npcm7xx_gc = {
	.owner			= THIS_MODULE,
	.label			= "npcmgpio",
...
static int npcm7xx_gpio_register(struct NPCM7xx_pinctrl *pctrl)
{
	...
	for_each_available_child_of_node(pctrl->dev->of_node, np)
		if (of_find_property(np, "gpio-controller", NULL)) {
			...
			pctrl->gpio_bank[id].gc = npcm7xx_gc;
