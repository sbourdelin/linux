...
static void npcmgpio_dbg_show(struct seq_file *s, struct gpio_chip *chip)
{
	...
	seq_printf(s, "DIN :%.8x DOUT:%.8x IE  :%.8x OE	 :%.8x\n",
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_DIN),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_DOUT),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_IEM),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_OE));
	seq_printf(s, "PU  :%.8x PD  :%.8x DB  :%.8x POL :%.8x\n",
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_PU),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_PD),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_DBNC),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_POL));
	seq_printf(s, "ETYP:%.8x EVBE:%.8x EVEN:%.8x EVST:%.8x\n",
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_EVTYP),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_EVBE),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_EVEN),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_EVST));
	seq_printf(s, "OTYP:%.8x OSRC:%.8x ODSC:%.8x\n",
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_OTYP),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_OSRC),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_ODSC));
	seq_printf(s, "OBL0:%.8x OBL1:%.8x OBL2:%.8x OBL3:%.8x\n",
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_OBL0),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_OBL1),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_OBL2),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_OBL3));
	seq_printf(s, "SLCK:%.8x MLCK:%.8x\n",
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_SPLCK),
		   (u32)bank->gc.read_reg(bank->base + NPCM7XX_GP_N_MPLCK));
}
...
static void npcmgpio_irq_handler(struct irq_desc *desc)
{
	...
	sts = gc->read_reg(bank->base + NPCM7XX_GP_N_EVST);
	en  = gc->read_reg(bank->base + NPCM7XX_GP_N_EVEN);
...
static void npcmgpio_irq_ack(struct irq_data *d)
{
	...
	bank->gc.write_reg(bank->base + NPCM7XX_GP_N_EVST, BIT(gpio));
}
...
static void npcmgpio_irq_mask(struct irq_data *d)
{
	...
	bank->gc.write_reg(bank->base + NPCM7XX_GP_N_EVENC, BIT(gpio));
}
...
static void npcmgpio_irq_unmask(struct irq_data *d)
{
	...
	bank->gc.write_reg(bank->base + NPCM7XX_GP_N_EVENS, BIT(gpio));
}
				 ...
				 struct regmap *gcr_regmap, unsigned int pin)
{
	...
	if (pincfg[pin].flag & SLEW)
		return bank->gc.read_reg(bank->base + NPCM7XX_GP_N_OSRC)
		& pinmask;
				      ...
				      unsigned int pin)
{
	...
	if (flg & DRIVE_STRENGTH_MASK) {
		...
		val = bank->gc.read_reg(bank->base + NPCM7XX_GP_N_ODSC)
		& pinmask;
				   ...
				   unsigned int offset, bool input)
{
	...
	dev_dbg(bank->gc.parent, "GPIO Set Direction: %d = %d\n", offset,
		input);
	if (input)
		bank->gc.write_reg(bank->base + NPCM7XX_GP_N_OEC, gpio);
	else
		bank->gc.write_reg(bank->base + NPCM7XX_GP_N_OES, gpio);
			      ...
			      unsigned long *config)
{
	...
	case PIN_CONFIG_BIAS_PULL_DOWN:
		pu = bank->gc.read_reg(bank->base + NPCM7XX_GP_N_PU) & pinmask;
		pd = bank->gc.read_reg(bank->base + NPCM7XX_GP_N_PD) & pinmask;
	...
	case PIN_CONFIG_INPUT_ENABLE:
		ie = bank->gc.read_reg(bank->base + NPCM7XX_GP_N_IEM) & pinmask;
		oe = bank->gc.read_reg(bank->base + NPCM7XX_GP_N_OE) & pinmask;
	...
	case PIN_CONFIG_DRIVE_PUSH_PULL:
		rc = !(bank->gc.read_reg(bank->base + NPCM7XX_GP_N_OTYP)
		       & pinmask);
		break;
	...
	case PIN_CONFIG_INPUT_DEBOUNCE:
		rc = bank->gc.read_reg(bank->base + NPCM7XX_GP_N_DBNC)
		& pinmask;
		break;
				  ...
				  unsigned int pin, unsigned long config)
{
	...
	case PIN_CONFIG_OUTPUT:
		...
		bank->gc.write_reg(arg ? bank->base + NPCM7XX_GP_N_DOS :
				   bank->base + NPCM7XX_GP_N_DOC, gpio);
		bank->gc.write_reg(bank->base + NPCM7XX_GP_N_OES, gpio);
		break;
	case PIN_CONFIG_DRIVE_PUSH_PULL:
...
static int npcm7xx_gpio_register(struct npcm7xx_pinctrl *pctrl)
{
	...
	for_each_available_child_of_node(pctrl->dev->of_node, np)
		if (of_find_property(np, "gpio-controller", NULL)) {
			...
			pctrl->gpio_bank[id].get_multiple =
				pctrl->gpio_bank[id].gc.get_multiple;
			pctrl->gpio_bank[id].gc.get_multiple =
				npcmgpio_get_multiple_value;
			...
			pctrl->gpio_bank[id].free =
				pctrl->gpio_bank[id].gc.free;
