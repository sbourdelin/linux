...
static const struct pin_config_item lochnagar_conf_items[] = {
	PCONFDUMP(PIN_CONFIG_AIFMASTER,  "cirrus,aif-master", NULL, false),
	PCONFDUMP(PIN_CONFIG_AIFSLAVE,   "cirrus,aif-slave",  NULL, false),
};
...
static int lochnagar_pin_probe(struct platform_device *pdev)
{
	...
	dev->of_node = of_node_get(lochnagar->dev->of_node);
...
int lochnagar_pin_remove(struct platform_device *pdev)
{
