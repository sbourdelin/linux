/*
 * Driver for STEC s1220 PCIe SSDs. STEC was acquired in 2013 by HGST and HGST
...
skip_sg:
	...
	if (skmsg != NULL) {
		...
		if (skmsg->length > sizeof(struct fit_msg_hdr)) {
			pr_debug("%s sending msg=%p, len %d\n", skdev->name,
				 skmsg, skmsg->length);
			skd_send_fitmsg(skdev, skmsg);
		} else {
			/*
			     ...
			     struct skd_request_context *skreq)
{
	...
	if (unlikely(skdev->dbg_level > 1)) {
		pr_debug("%s skreq=%x sksg_list=%p sksg_dma=%llx\n",
			 skdev->name, skreq->id, skreq->sksg_list,
			 skreq->sksg_dma_address);
		for (i = 0; i < n_sg; i++) {
			struct fit_sg_descriptor *sgd = &skreq->sksg_list[i];
			pr_debug("%s   sg[%d] count=%u ctrl=0x%x addr=0x%llx next=0x%llx\n",
				 skdev->name, i, sgd->byte_count, sgd->control,
...
static void skd_timer_tick(ulong arg)
{
	...
	pr_debug("%s found %d timeouts, draining busy=%d\n", skdev->name,
...
static void skd_timer_tick_not_online(struct skd_device *skdev)
{
	...
	case SKD_DRVR_STATE_LOAD:
		break;
	case SKD_DRVR_STATE_BUSY_SANITIZE:
		pr_debug("%s drive busy sanitize[%x], driver[%x]\n",
			 skdev->name, skdev->drive_state, skdev->state);
	...
	case SKD_DRVR_STATE_BUSY_IMMINENT:
	case SKD_DRVR_STATE_BUSY_ERASE:
		pr_debug("%s busy[%x], countdown=%d\n", skdev->name,
			 skdev->state, skdev->timer_countdown);
		if (skdev->timer_countdown > 0) {
			skdev->timer_countdown--;
			return;
		}
		pr_debug("%s busy[%x], timedout=%d, restarting device.",
			 skdev->name, skdev->state, skdev->timer_countdown);
	...
	case SKD_DRVR_STATE_DRAINING_TIMEOUT:
		pr_debug("%s draining busy [%d] tick[%d] qdb[%d] tmls[%d]\n",
			 skdev->name, skdev->timo_slot, skdev->timer_countdown,
			 skdev->in_flight,
			 skdev->timeout_slot[skdev->timo_slot]);
		/* if the slot has cleared we can let the I/O continue */
		if (skdev->timeout_slot[skdev->timo_slot] == 0) {
			pr_debug("%s Slot drained, starting queue.\n",
				 skdev->name);
			  ...
			  uint cmd_in, ulong arg)
{
	...
	pr_debug("%s %s: CMD[%s] ioctl  mode 0x%x, cmd 0x%x arg %0lx\n",
		 skdev->name, disk->disk_name, current->comm, mode, cmd_in,
		 arg);
	...
	pr_debug("%s %s:  completion rc %d\n", skdev->name, disk->disk_name,
		 rc);
	return rc;
}
					...
					struct skd_sg_io *sksgio)
{
	...
	if (!access_ok(VERIFY_WRITE, sksgio->argp, sizeof(sg_io_hdr_t))) {
		pr_debug("%s access sg failed %p\n", skdev->name, sksgio->argp);
		return -EFAULT;
	}

	if (__copy_from_user(sgp, sksgio->argp, sizeof(sg_io_hdr_t))) {
		pr_debug("%s copy_from_user sg failed %p\n", skdev->name,
			 sksgio->argp);
		return -EFAULT;
	}

	if (sgp->interface_id != SG_INTERFACE_ID_ORIG) {
		pr_debug("%s interface_id invalid 0x%x\n", skdev->name,
			 sgp->interface_id);
		return -EINVAL;
	}

	if (sgp->cmd_len > sizeof(sksgio->cdb)) {
		pr_debug("%s cmd_len invalid %d\n", skdev->name, sgp->cmd_len);
		return -EINVAL;
	}

	if (sgp->iovec_count > 256) {
		pr_debug("%s iovec_count invalid %d\n", skdev->name,
			 sgp->iovec_count);
		return -EINVAL;
	}

	if (sgp->dxfer_len > (PAGE_SIZE * SKD_N_SG_PER_SPECIAL)) {
		pr_debug("%s dxfer_len invalid %d\n", skdev->name,
			 sgp->dxfer_len);
		return -EINVAL;
	}
	...
	default:
		pr_debug("%s dxfer_dir invalid %d\n", skdev->name,
			 sgp->dxfer_direction);
		return -EINVAL;
	}

	if (copy_from_user(sksgio->cdb, sgp->cmdp, sgp->cmd_len)) {
		pr_debug("%s copy_from_user cmdp failed %p\n", skdev->name,
			 sgp->cmdp);
		return -EFAULT;
	}
	...
	if (sgp->mx_sb_len != 0) {
		if (!access_ok(VERIFY_WRITE, sgp->sbp, sgp->mx_sb_len)) {
			pr_debug("%s access sbp failed %p\n", skdev->name,
				 sgp->sbp);
			return -EFAULT;
		}
	}
	...
	if (sgp->iovec_count == 0) {
		...
		if (iov == NULL) {
			pr_debug("%s alloc iovec failed %d\n", skdev->name,
				 sgp->iovec_count);
			return -ENOMEM;
		}
		sksgio->iov = iov;
		...
		if (copy_from_user(iov, sgp->dxferp, nbytes)) {
			pr_debug("%s copy_from_user iovec failed %p\n",
				 skdev->name, sgp->dxferp);
			return -EFAULT;
		}
	...
	if (sgp->dxfer_direction != SG_DXFER_NONE) {
		...
		for (i = 0; i < sksgio->iovcnt; i++, iov++) {
			if (!access_ok(acc, iov->iov_base, iov->iov_len)) {
				pr_debug("%s access data failed %p/%zd\n",
					 skdev->name, iov->iov_base,
					 iov->iov_len);
				return -EFAULT;
			}
		}
...
static int skd_sg_io_await(struct skd_device *skdev, struct skd_sg_io *sksgio)
{
	...
	if (sksgio->skspcl->req.state == SKD_REQ_STATE_ABORTED) {
		pr_debug("%s skspcl %p aborted\n", skdev->name, sksgio->skspcl);
				...
				struct skd_sg_io *sksgio)
{
	...
	pr_debug("%s status %x masked %x resid 0x%x\n", skdev->name,
	...
	if (sgp->masked_status == SAM_STAT_CHECK_CONDITION) {
		if (sgp->mx_sb_len > 0) {
			...
			if (__copy_to_user(sgp->sbp, ei, nbytes)) {
				pr_debug("%s copy_to_user sense failed %p\n",
					 skdev->name, sgp->sbp);
				return -EFAULT;
			}
		}
	...
	if (__copy_to_user(sksgio->argp, sgp, sizeof(sg_io_hdr_t))) {
		pr_debug("%s copy_to_user sg failed %p\n", skdev->name,
			 sksgio->argp);
		return -EFAULT;
	}
				  ...
				  struct skd_special_context *skspcl)
{
	...
	pr_debug("%s complete internal %x\n", skdev->name, scsi->cdb[0]);
	...
	case TEST_UNIT_READY:
		if (status == SAM_STAT_GOOD)
			...
			if (skdev->state == SKD_DRVR_STATE_STOPPING) {
				pr_debug("%s TUR failed, don't send anymore state 0x%x\n",
					 skdev->name, skdev->state);
				return;
			}
			pr_debug("%s **** TUR failed, retry skerr\n",
				 skdev->name);
	...
	case WRITE_BUFFER:
		if (status == SAM_STAT_GOOD)
			...
			if (skdev->state == SKD_DRVR_STATE_STOPPING) {
				pr_debug("%s write buffer failed, don't send anymore state 0x%x\n",
					 skdev->name, skdev->state);
				return;
			}
			pr_debug("%s **** write buffer failed, retry skerr\n",
				 skdev->name);
			skd_send_internal_skspcl(skdev, skspcl, 0x00);
		}
		break;
	...
	case READ_BUFFER:
		if (status == SAM_STAT_GOOD) {
			...
			if (skdev->state == SKD_DRVR_STATE_STOPPING) {
				pr_debug("%s read buffer failed, don't send anymore state 0x%x\n",
					 skdev->name, skdev->state);
				return;
			}
			pr_debug("%s **** read buffer failed, retry skerr\n",
				 skdev->name);
			skd_send_internal_skspcl(skdev, skspcl, 0x00);
		}
		break;
	...
	case READ_CAPACITY:
		...
		if (status == SAM_STAT_GOOD) {
			...
			set_capacity(skdev->disk, skdev->read_cap_last_lba + 1);
			pr_debug("%s **** MEDIUM ERROR caused READCAP to fail, ignore failure and continue to inquiry\n",
				 skdev->name);
			skd_send_internal_skspcl(skdev, skspcl, INQUIRY);
		} else {
			pr_debug("%s **** READCAP failed, retry TUR\n",
				 skdev->name);
			skd_send_internal_skspcl(skdev, skspcl,
						 TEST_UNIT_READY);
		}
	...
	case INQUIRY:
		...
		if (skd_unquiesce_dev(skdev) < 0)
			pr_debug("%s **** failed, to ONLINE device\n",
				 skdev->name);
			    ...
			    struct skd_fitmsg_context *skmsg)
{
	...
	pr_debug("%s dma address 0x%llx, busy=%d\n", skdev->name,
		 skmsg->mb_dma_address, skdev->in_flight);
	pr_debug("%s msg_buf 0x%p, offset %x\n", skdev->name, skmsg->msg_buf,
		 skmsg->offset);
				    ...
				    struct skd_special_context *skspcl)
{
	...
	if (unlikely(skdev->dbg_level > 1)) {
		...
		for (i = 0; i < SKD_N_SPECIAL_FITMSG_BYTES; i += 8) {
			pr_debug("%s  spcl[%2d] %8ph\n", skdev->name, i,
				 &bp[i]);
			if (i == 0)
				i = 64 - 8;
		}

		pr_debug("%s skspcl=%p id=%04x sksg_list=%p sksg_dma=%llx\n",
			 skdev->name, skspcl, skspcl->req.id,
			 skspcl->req.sksg_list, skspcl->req.sksg_dma_address);
		 ...
		 u8 cmp_status, volatile struct fit_comp_error_info *skerr)
{
	...
	pr_debug("%s stat: t=%02x stat=%02x k=%02x c=%02x q=%02x fruc=%02x\n",
		 skdev->name, skerr->type, cmp_status, skerr->key, skerr->code,
		 skerr->qual, skerr->fruc);
	 ...
	 */
	if (cmp_status) {
		pr_debug("%s status check: error\n", skdev->name);
		return SKD_CHECK_STATUS_REPORT_ERROR;
	}

	pr_debug("%s status check good default\n", skdev->name);
			       ...
			       uint8_t *cdb, uint8_t *buf)
{
	 ...
	 * its page.
	 */
	pr_debug("%s skd_do_driver_inquiry: modify supported pages.\n",
		 skdev->name);
			       ...
			       uint8_t *cdb, uint8_t *buf)
{
	...
	pr_debug("%s skd_do_driver_inquiry: return driver page\n", skdev->name);
					...
					int limit, int *enqueued)
{
	...
	for (;; ) {
		...
		pr_debug("%s cycle=%d ix=%d got cycle=%d cmdctxt=0x%x stat=%d busy=%d rbytes=0x%x proto=%d\n",
			 skdev->name, skdev->skcomp_cycle, skdev->skcomp_ix,
			 cmp_cycle, cmp_cntxt, cmp_status, skdev->in_flight,
			 cmp_bytes, skdev->proto_ver);

		if (cmp_cycle != skdev->skcomp_cycle) {
			pr_debug("%s end of completions\n", skdev->name);
			break;
		}
		/*
		 ...
		 */
		if (skreq->id != req_id) {
			pr_debug("%s mismatch comp_id=0x%x req_id=0x%x\n",
				 skdev->name, req_id, skreq->id);
			{
				u16 new_id = cmp_cntxt;
				pr_err("(%s): Completion mismatch comp_id=0x%04x skreq=0x%04x new=0x%04x\n",
				       skd_name(skdev), req_id, skreq->id, new_id);
		...
		if (skreq->state == SKD_REQ_STATE_ABORTED) {
			pr_debug("%s reclaim req %p id=%04x\n",
				 skdev->name,
				 skreq, skreq->id);
		...
		if (!skreq->req) {
			pr_debug("%s NULL backptr skdreq %p, req=0x%x req_id=0x%x\n",
				 skdev->name, skreq, skreq->id, req_id);
		} else {
			/*
			       ...
			       volatile struct fit_comp_error_info *skerr)
{
	...
	pr_debug("%s table=0x%x id=0x%x slot=%d\n", skdev->name, req_table,
		 req_id, req_slot);
...
static void skd_complete_special(struct skd_device *skdev,
				 ...
				 *skcomp,
				 ...
				 struct skd_special_context *skspcl)
{
	pr_debug("%s  completing special request %p\n", skdev->name, skspcl);
	if (skspcl->orphaned) {
		/* Discard orphaned request */
		/* ?: Can this release directly or does it need
		 * to use a worker? */
		pr_debug("%s release orphaned %p\n", skdev->name, skspcl);
		skd_release_special(skdev, skspcl);
		return;
	}
				...
				struct skd_special_context *skspcl)
{
	...
	if (was_depleted) {
		pr_debug("%s skspcl was depleted\n", skdev->name);
...
skd_isr(int irq, void *ptr)
{
	...
	for (;; ) {
		...
		pr_debug("%s intstat=0x%x ack=0x%x\n", skdev->name, intstat,
			 ack);
...
static void skd_isr_fwstate(struct skd_device *skdev)
{
	...
	case FIT_SR_DRIVE_FW_BOOTING:
		pr_debug("%s ISR FIT_SR_DRIVE_FW_BOOTING %s\n", skdev->name,
			 skdev->name);
...
static void skd_recover_requests(struct skd_device *skdev, int requeue)
{
	...
	for (i = 0; i < skdev->n_special; i++) {
		...
		if (skspcl->req.state == SKD_REQ_STATE_BUSY) {
			if (skspcl->orphaned) {
				pr_debug("%s orphaned %p\n", skdev->name,
					 skspcl);
				skd_release_special(skdev, skspcl);
			} else {
				pr_debug("%s not orphaned %p\n", skdev->name,
					 skspcl);
...
static void skd_isr_msg_from_dev(struct skd_device *skdev)
{
	...
	pr_debug("%s mfd=0x%x last_mtd=0x%x\n", skdev->name, mfd,
		 skdev->last_mtd);
...
static void skd_enable_interrupts(struct skd_device *skdev)
{
	...
	pr_debug("%s interrupt mask=0x%x\n", skdev->name, ~val);

	val = SKD_READL(skdev, FIT_CONTROL);
	val |= FIT_CR_ENABLE_INTERRUPTS;
	pr_debug("%s control=0x%x\n", skdev->name, val);
...
static void skd_start_device(struct skd_device *skdev)
{
	...
	pr_debug("%s initial status=0x%x\n", skdev->name, sense);
	...
	case FIT_SR_DRIVE_FW_BOOTING:
		pr_debug("%s FIT_SR_DRIVE_FW_BOOTING %s\n", skdev->name,
			 skdev->name);
		skdev->state = SKD_DRVR_STATE_WAIT_BOOT;
		skdev->timer_countdown = SKD_WAIT_BOOT_TIMO;
		break;
	...
	case 0xFF:
		...
		skd_drive_disappeared(skdev);
		/*start the queue so we can respond with error to requests */
		pr_debug("%s starting %s queue to error-out reqs\n",
			 skdev->name, skdev->name);
		blk_start_queue(skdev->queue);
		skdev->gendisk_on = -1;
		wake_up_interruptible(&skdev->waitq);
	...
	pr_debug("%s FIT Control Status=0x%x\n", skdev->name, state);

	state = SKD_READL(skdev, FIT_INT_STATUS_HOST);
	pr_debug("%s Intr Status=0x%x\n", skdev->name, state);

	state = SKD_READL(skdev, FIT_INT_MASK_HOST);
	pr_debug("%s Intr Mask=0x%x\n", skdev->name, state);

	state = SKD_READL(skdev, FIT_MSG_FROM_DEVICE);
	pr_debug("%s Msg from Dev=0x%x\n", skdev->name, state);

	state = SKD_READL(skdev, FIT_HW_VERSION);
	pr_debug("%s HW version=0x%x\n", skdev->name, state);
...
static int skd_quiesce_dev(struct skd_device *skdev)
{
	...
	default:
		rc = -EINVAL;
		pr_debug("%s state [%d] not implemented\n", skdev->name,
			 skdev->state);
	}
	return rc;
}
...
static int skd_unquiesce_dev(struct skd_device *skdev)
{
	...
	if (skdev->state == SKD_DRVR_STATE_ONLINE) {
		pr_debug("%s **** device already ONLINE\n", skdev->name);
		return 0;
	}
	...
	case SKD_DRVR_STATE_LOAD:
		...
		pr_err("(%s): Driver state %s(%d)=>%s(%d)\n",
		       ...
		       skdev->state);
		pr_debug("%s **** device ONLINE...starting block queue\n",
			 skdev->name);
		pr_debug("%s starting %s queue\n", skdev->name, skdev->name);
	...
	case SKD_DRVR_STATE_DISAPPEARED:
	default:
		pr_debug("%s **** driver state %d, not implemented \n",
			 skdev->name, skdev->state);
		return -EBUSY;
	}
	return 0;
...
static int skd_acquire_msix(struct skd_device *skdev)
{
	...
	pr_debug("%s %s: <%s> msix %d irq(s) enabled\n", skdev->name,
...
static int skd_cons_skcomp(struct skd_device *skdev)
{
	...
	pr_debug("%s comp pci_alloc, total bytes %d entries %d\n", skdev->name,
...
static int skd_cons_skmsg(struct skd_device *skdev)
{
	...
	pr_debug("%s skmsg_table kzalloc, struct %lu, count %u total %lu\n",
		 skdev->name, sizeof(struct skd_fitmsg_context),
...
static int skd_cons_skreq(struct skd_device *skdev)
{
	...
	pr_debug("%s skreq_table kzalloc, struct %lu, count %u total %lu\n",
		 skdev->name, sizeof(struct skd_request_context),
	...
	pr_debug("%s alloc sg_table sg_per_req %u scatlist %lu total %lu\n",
		 skdev->name,
...
static int skd_cons_skspcl(struct skd_device *skdev)
{
	...
	pr_debug("%s skspcl_table kzalloc, struct %lu, count %u total %lu\n",
		 skdev->name, sizeof(struct skd_special_context),
		 skdev->n_special,
...
static int skd_bdev_getgeo(struct block_device *bdev, struct hd_geometry *geo)
{
	...
	pr_debug("%s %s: CMD[%s] getgeo device\n", skdev->name,
...
static int skd_pci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	for (i = 0; i < SKD_MAX_BARS; i++) {
		...
		pr_debug("%s mem_map=%p, phyd=%016llx, size=%d\n", skdev->name,
...
static void skd_log_skdev(struct skd_device *skdev, const char *event)
{
	pr_debug("%s (%s) skdev=%p event='%s'\n", skdev->name, skdev->name,
		 skdev, event);
	pr_debug("%s   drive_state=%s(%d) driver_state=%s(%d)\n", skdev->name,
		 skd_drive_state_to_str(skdev->drive_state), skdev->drive_state,
		 skd_skdev_state_to_str(skdev->state), skdev->state);
	pr_debug("%s   busy=%d limit=%d dev=%d lowat=%d\n", skdev->name,
		 skdev->in_flight, skdev->cur_max_queue_depth,
		 skdev->dev_max_queue_depth, skdev->queue_low_water_mark);
	pr_debug("%s   timestamp=0x%x cycle=%d cycle_ix=%d\n", skdev->name,
...
static void skd_log_skmsg(struct skd_device *skdev,
			  struct skd_fitmsg_context *skmsg, const char *event)
{
	pr_debug("%s (%s) skmsg=%p event='%s'\n", skdev->name, skdev->name,
		 skmsg, event);
	pr_debug("%s   state=%s(%d) id=0x%04x length=%d\n", skdev->name,
...
static void skd_log_skreq(struct skd_device *skdev,
			  struct skd_request_context *skreq, const char *event)
{
	pr_debug("%s (%s) skreq=%p event='%s'\n", skdev->name, skdev->name,
		 skreq, event);
	pr_debug("%s   state=%s(%d) id=0x%04x fitmsg=0x%04x\n",
		 skdev->name, skd_skreq_state_to_str(skreq->state),
		 skreq->state, skreq->id, skreq->fitmsg_id);
	pr_debug("%s   timo=0x%x sg_dir=%d n_sg=%d\n", skdev->name,
	...
	if (skreq->req != NULL) {
		...
		pr_debug("%s req=%p lba=%u(0x%x) count=%u(0x%x) dir=%d\n",
			 skdev->name, req, lba, lba, count, count,
			 (int)rq_data_dir(req));
	} else
		pr_debug("%s req=NULL\n", skdev->name);
}
