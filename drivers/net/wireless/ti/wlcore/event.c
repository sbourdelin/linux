...
int wlcore_event_fw_logger(struct wl1271 *wl)
{
	...
	if (fw_log.actual_buff_size == 0)
		goto free_out;

	actual_len = fw_log.actual_buff_size;
	start_loc = (fw_log.buff_read_ptr - internal_fw_addrbase) - addr;
	end_buff_addr += fw_log.max_buff_size;
	available_len = end_buff_addr -
			(fw_log.buff_read_ptr - internal_fw_addrbase);
	actual_len = min(actual_len, available_len);
	len = actual_len;

	wl12xx_copy_fwlog(wl, &buffer[start_loc], len);
	clear_addr = addr + start_loc + fw_log.actual_buff_size +
			internal_fw_addrbase;

	len = fw_log.actual_buff_size - len;
	if (len) {
	...
	if (clear_addr != fw_log.buff_write_ptr) {
		wl1271_error("Calculate of clear addr Clear = %x, write = %x",
			     clear_addr, fw_log.buff_write_ptr);
	}
...
free_out:
	kfree(buffer);
out:
	return fw_log.actual_buff_size;
}
