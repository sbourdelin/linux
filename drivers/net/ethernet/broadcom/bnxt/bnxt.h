...
int bnxt_setup_tc(struct net_device *dev, u8 tc);

extern const struct dcbnl_rtnl_ops dcbnl_ops;
#define BNXT_MAX_USER_PRIORITY 8
#define BNXT_MAX_TC            8
#define BNXT_MAX_RINGS_PER_TC  8
#define BNXT_ETS_MAX_BW        100
#define BNXT_FCOE_DEFTC        3

struct bnxt_dcb {
	/* maximum number of traffic class */
	u8			max_tc;
	/* priority to traffic class mapping */
	u8			prio_tc[B_MAX_TC];
	struct ieee_pfc	*b_ieee_pfc;
	struct ieee_ets	*b_ieee_ets;
	u8			dcbx_cap;
	u8                      fcoe_pri;
	u8                      default_pri;
};
...
#define BNXT_FIRST_VF_FID	128
	...
	u16	max_rsscos_ctxs;
	u16	max_cp_rings;
	u16	max_pf_tx_rings; /* HW assigned max tx rings for this PF */
	u16	max_tx_rings; /* runtime max tx rings for this PF */
	u16	max_pf_rx_rings; /* HW assigned max rx rings for this PF */
	u16	max_rx_rings; /* runtime max tx rings for this PF */
	u16	max_irqs;
	u16	max_l2_ctxs;
	u16	max_vnics;
	u16	max_stat_ctxs;
...
#define NUM_REG_WINDOWS		16
	u32			reg_window_base[NUM_REG_WINDOWS];
#ifdef CONFIG_BNXT_DCB
	struct bnxt_dcb            dcb;
#endif /* CONFIG_BNXT_DCB */
...
#ifdef BNXT_FPGA
#define BNXT_USEC_TO_COAL_TIMER(x)	(((x) < 10) ? 1 : (x) / 10)
#define BNXT_COAL_TIMER_TO_USEC(x) ((x) * 10)
#else
