	...
	struct init_qm_vport_params	*vport_params)
{
	...
	for (i = 0, pq_id = start_pq; i < num_pqs; i++, pq_id++) {
		...
		u16 first_tx_pq_id = pq_ids[pq_params[i].tc_id];

		if (first_tx_pq_id == QM_INVALID_PQ_ID) {
			/* create new VP PQ */
			pq_ids[pq_params[i].tc_id] = pq_id;
...
{
	u8 tc, i;
	u32 other_mem_size_4kb = QM_PQ_MEM_4KB(num_pf_cids + num_tids) *
				 QM_OTHER_PQS_PER_PF;
	...
	qed_other_pq_map_rt_init(p_hwfn, port_id, pf_id, num_pf_cids,
				 num_tids, 0);

	/* map Tx PQs */
	qed_tx_pq_map_rt_init(p_hwfn, p_ptt, port_id, pf_id,
			      max_phys_tcs_per_port, is_first_pf,
			      num_pf_cids, num_vf_cids, start_pq,
