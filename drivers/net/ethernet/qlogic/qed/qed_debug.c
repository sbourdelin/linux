				   ...
				   struct dbg_attn_block_result *results)
{
	...
	block_attn_name_offsets =
	    &s_user_dbg_arrays[BIN_BUF_DBG_ATTN_NAME_OFFSETS].
	    ptr[results->names_offset];
	...
	for (i = 0; i < num_regs; i++) {
		...
		bit_mapping =
		    &((struct dbg_attn_bit_mapping *)
		      s_user_dbg_arrays[BIN_BUF_DBG_ATTN_INDEXES].
		      ptr)[reg_result->block_attn_offset];
		...
		for (j = 0; j < num_reg_attn; j++) {
			...
			if (reg_result->sts_val & BIT(bit_idx)) {
				...
				attn_name =
				    &((const char *)
				      s_user_dbg_arrays
				      [BIN_BUF_DBG_PARSING_STRINGS].ptr)
				    [attn_name_offset];
				attn_type_str =
				    (attn_type ==
