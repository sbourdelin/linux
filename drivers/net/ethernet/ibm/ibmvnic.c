...
static int ibmvnic_max_tx_queues = IBMVNIC_MAX_TX_QUEUES;
module_param(ibmvnic_max_tx_queues, int, S_IRUGO);
MODULE_PARM_DESC(ibmvnic_max_tx_queues, "IBMVNIC max number of tx queues");

static int ibmvnic_req_mtu;
module_param(ibmvnic_req_mtu, int, 0);
MODULE_PARM_DESC(ibmvnic_req_mtu,
		 "IBMVNIC requested MTU, (0 = max allowed by hw)");

static int long_term_map = 1;
module_param(long_term_map, int, 0);
MODULE_PARM_DESC(long_term_map, "Enable/disable Long term mapping");
			 ...
			 struct ibmvnic_rx_pool *pool)
{
	...
	if (long_term_map) {
		pool->rx_lb_size = pool->size * pool->buff_size;
		pool->rx_long_term_buff =
			dma_alloc_coherent(dev, pool->rx_lb_size,
					   &pool->rx_lb_addr, GFP_KERNEL);

		if (!pool->rx_long_term_buff) {
			dev_err(dev, "Couldn't alloc rx long term buffer\n");
			return -ENOMEM;
		}

		send_request_map(adapter, pool->rx_lb_addr, pool->rx_lb_size,
				 adapter->map_id);
		pool->map_id = adapter->map_id;
		adapter->map_id++;
			      ...
			      struct ibmvnic_rx_pool *pool)
{
	...
	for (i = 0; i < count; ++i) {
		...
		if (!long_term_map) {
			...
			int offset = index * pool->buff_size;
			unsigned char *dst =
					pool->rx_long_term_buff + offset;
			memset(dst, 0, pool->buff_size);
			skb_copy_from_linear_data(skb, dst, pool->buff_size);
			dma_addr = pool->rx_lb_addr + offset;
		...
		sub_crq.rx_add.map_id = pool->map_id;

		/* The PAPR defines the length field of the
		 * sCRQ to be 24 bits so the buffer size needs to be left
		 * shifted by a byte before it is converted to BE.
		 */
	...
	for (i = 0; i < tx_subcrqs; i++) {
		...
		if (long_term_map) {
			tx_pool->tx_lb_size =
				...
				adapter->req_mtu;
			tx_pool->tx_long_term_buff =
				dma_alloc_coherent(dev, tx_pool->tx_lb_size,
						   &tx_pool->tx_lb_addr,
						   GFP_KERNEL);

			if (!tx_pool->tx_long_term_buff) {
				dev_err(dev, "Couldn't alloc rx long term buffer\n");
				goto tx_pool_alloc_failed;
			}

			send_request_map(adapter, tx_pool->tx_lb_addr,
					 tx_pool->tx_lb_size, adapter->map_id);
			tx_pool->map_id = adapter->map_id;
			adapter->map_id++;
			init_completion(&adapter->fw_done);
			wait_for_completion(&adapter->fw_done);
		}
...
tx_pool_alloc_failed:
	for (j = 0; j < i; j++) {
		...
		if (long_term_map) {
			dma_free_coherent(dev, adapter->tx_pool[j].tx_lb_size,
					  adapter->tx_pool[j].tx_long_term_buff,
					  adapter->tx_pool[j].tx_lb_addr);
			send_request_unmap(adapter, adapter->tx_pool[i].map_id);
		}
	}
...
rx_pool_alloc_failed:
	for (j = 0; j < i; j++) {
		...
		if (long_term_map) {
			dma_free_coherent(dev, adapter->rx_pool[j].rx_lb_size,
					  adapter->rx_pool[j].rx_long_term_buff,
					  adapter->rx_pool[j].rx_lb_addr);
			send_request_unmap(adapter, adapter->rx_pool[i].map_id);
		}
	}
...
static int ibmvnic_close(struct net_device *netdev)
{
	...
	for (i = 0; i < be32_to_cpu(adapter->login_rsp_buf->num_txsubm_subcrqs);
		...
		if (long_term_map) {
			dma_free_coherent(dev, adapter->tx_pool[i].tx_lb_size,
					  adapter->tx_pool[i].tx_long_term_buff,
					  adapter->tx_pool[i].tx_lb_addr);
			send_request_unmap(adapter, adapter->tx_pool[i].map_id);
		}
	}
	kfree(adapter->tx_pool);
	...
	for (i = 0; i < be32_to_cpu(adapter->login_rsp_buf->num_rxadd_subcrqs);
		...
		if (long_term_map) {
			dma_free_coherent(dev, adapter->rx_pool[i].rx_lb_size,
					  adapter->rx_pool[i].rx_long_term_buff,
					  adapter->rx_pool[i].rx_lb_addr);
			send_request_unmap(adapter, adapter->rx_pool[i].map_id);
		}
	}
	kfree(adapter->rx_pool);
...
static int ibmvnic_xmit(struct sk_buff *skb, struct net_device *netdev)
{
	...
	struct ibmvnic_tx_pool *tx_pool = &adapter->tx_pool[queue_num];
	...
	if (!long_term_map) {
		data_dma_addr =
			dma_map_single(dev, skb->data, skb->len,
				       DMA_TO_DEVICE);
		...
		int offset = index * adapter->req_mtu;
		unsigned char *dst =
				tx_pool->tx_long_term_buff + offset;
		memset(dst, 0, adapter->req_mtu);
		skb_copy_from_linear_data(skb, dst, skb->len);
		data_dma_addr = tx_pool->tx_lb_addr + offset;
	}
	...
	tx_crq.v1.dma_reg = cpu_to_be16(tx_pool->map_id);
				  ...
				  struct ibmvnic_rx_buff *rx_buff)
{
	...
	if (!long_term_map)
		dma_unmap_single(dev, rx_buff->dma,
				 rx_buff->size, DMA_FROM_DEVICE);
...
restart_poll:
	while (frames_processed < budget) {
		...
		/* TODO handle vlan_tci */
		vlan_tci = be16_to_cpu(next->rx_comp.vlan_tci);
		skb = rx_buff->skb;
		if (long_term_map)
...
static const struct net_device_ops ibmvnic_netdev_ops = {
	...
	.ndo_do_ioctl		= ibmvnic_ioctl,
	.ndo_change_mtu		= ibmvnic_change_mtu,
	.ndo_tx_timeout		= ibmvnic_tx_timeout,
	.ndo_get_stats		= ibmvnic_get_stats,
#ifdef CONFIG_NET_POLL_CONTROLLER
	...
	while (pending_scrq(adapter, scrq)) {
		...
		for (i = 0; i < next->tx_comp.num_comps; i++) {
			...
			for (j = 0; j < IBMVNIC_MAX_FRAGS_PER_CRQ; j++) {
				...
				if (!txbuff->used_bounce && !long_term_map)
...
static void init_sub_crqs(struct ibmvnic_adapter *adapter, int retry)
{
	...
	if (!retry) {
		...
		if (ibmvnic_req_mtu) {
			if (ibmvnic_req_mtu > adapter->max_mtu ||
			    ibmvnic_req_mtu < adapter->min_mtu)
				dev_warn(dev, "Module param req_mtu out of range, ignoring it\n");
			else
				adapter->req_mtu = ibmvnic_req_mtu;
		}
	}
				   ...
				   struct ibmvnic_adapter *adapter)
{
	...
	if (rc) {
		...
		adapter->map_id--;
		/* need to find and zero tx_pool map_id */
		for (i = 0; i < tx_subcrqs; i++) {
			if (adapter->tx_pool[i].map_id == map_id)
				adapter->tx_pool[i].map_id = 0;
		}
		return;
	}
	complete(&adapter->fw_done);
				 ...
				 struct ibmvnic_adapter *adapter)
{
	...
	case MAX_TX_QUEUES:
		...
		netdev_dbg(netdev, "max_tx_queues = %lld\n",
			   adapter->max_tx_queues);
		if (adapter->max_tx_queues > ibmvnic_max_tx_queues)
			netdev_warn(netdev, "Adapter's max_tx_queues is greater than module parameter. Set module paramenter to %lld, for optimum performance\n",
				    adapter->max_tx_queues);
		break;
	case MAX_RX_QUEUES:
