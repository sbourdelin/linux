...
static int lan743x_csr_init(struct lan743x_adapter *adapter)
{
	...
	csr->csr_address = devm_ioremap(&adapter->pci.pdev->dev,
				     ...
				     void *context)
{
	...
	ret = devm_request_irq(&adapter->pci.pdev->dev, vector->irq,
...
static int lan743x_mdiobus_init(struct lan743x_adapter *adapter)
{
	...
	adapter->mdiobus = devm_mdiobus_alloc(&adapter->pci.pdev->dev);
	if (!(adapter->mdiobus)) {
		ret = -ENOMEM;
		goto clean_up;
	}
