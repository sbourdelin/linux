...
static void cpts_calc_mult_shift(struct cpts *cpts)
{
	...
	clocks_calc_mult_shift(&cpts->tc.cc.mult, &cpts->tc.cc.shift,
			       freq, NSEC_PER_SEC, maxsec);

	frac = 0;
	ns = cyclecounter_cyc2ns(&cpts->tc.cc, freq, cpts->tc.cc.mask, &frac);

	dev_info(cpts->dev,
		 "CPTS: ref_clk_freq:%u calc_mult:%u calc_shift:%u error:%lld nsec/sec\n",
		 freq, cpts->tc.cc.mult, cpts->tc.cc.shift,
		 (ns - NSEC_PER_SEC));
}
