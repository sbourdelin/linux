...
#define LM_SUP BIT(0)
#define LM_ADV BIT(1)
#define FLG (LM_SUP | LM_ADV)
	u32 hns3_link_mode;
	u32 ethtool_link_mode;
	u32 link_mode_ksettings;
};
					 ...
					 struct ethtool_link_ksettings *cmd)
{
	...
	for (i = 0; i < ARRAY_SIZE(hns3_lm_map); i++) {
		...
		if (hns3_lm_map[i].link_mode_ksettings & LM_SUP) {
		...
		if (hns3_lm_map[i].link_mode_ksettings & LM_ADV) {
			ethtool_link_ksettings_zero_link_mode(cmd,
							      advertising);
			__set_bit(hns3_lm_map[i].ethtool_link_mode,
				   ...
				   struct ethtool_link_ksettings *cmd)
{
	...
	hns3_driv_to_eth_caps(supported_caps, cmd);
