...
static int hi3110_set_normal_mode(struct spi_device *spi)
{
	...
	if (priv->can.ctrlmode & CAN_CTRLMODE_BERR_REPORTING)
		reg = HI3110_INT_BUSERR;

	hi3110_write(spi, HI3110_WRITE_INTE, reg |
...
static void hi3110_restart_work_handler(struct work_struct *ws)
{
	...
	if (priv->restart_tx) {
		...
		hi3110_clean(net);
		netif_wake_queue(net);
		hi3110_error_skb(net, CAN_ERR_RESTARTED, 0, 0, 0);
	}
	mutex_unlock(&priv->hi3110_lock);
}
...
static irqreturn_t hi3110_can_ist(int irq, void *dev_id)
{
	...
	while (!priv->force_quit) {
		...
		u8 intf;
		u8 eflag;
		int can_id = 0, data1 = 0, data2 = 0, data3 = 0;
		...
		if (eflag & HI3110_ERR_PROTOCOL_MASK) {
			...
			if (eflag & HI3110_ERR_BITERR)
				...
				data3 |= CAN_ERR_PROT_LOC_CRC_SEQ;
			else if (eflag & HI3110_ERR_ACKERR)
				data3 |= CAN_ERR_PROT_LOC_ACK;
		}
		...
		if (intf & HI3110_INT_BUSERR) {
			/* Note: HI3110 Does not report overflow errors */
			netdev_dbg(priv->net, "Bus Error\n");
			hi3110_error_skb(net, can_id, data1, data2, data3);
		}
