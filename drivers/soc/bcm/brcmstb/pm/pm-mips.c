...
{
	/*
	 * We need to pass 6 arguments to an assembly function. Lets avoid
	 * the stack and pass arguments in a explicit 4 byte array. The
	 * assembly code assumes all arguements are 4 bytes and arguments
	 * are ordered like so:
	 *
