					   ...
					   struct cmdq_task_cb cb)
{
	...
	struct device *dev = cmdq->mbox.dev;
	struct cmdq_task *task;

	task = kzalloc(sizeof(*task), GFP_KERNEL);
	INIT_LIST_HEAD(&task->list_entry);
	task->va_base = kmalloc(rec->cmd_buf_size, GFP_KERNEL);
	if (!task->va_base) {
	...
	task->cmdq = cmdq;
	task->cmd_buf_size = rec->cmd_buf_size;
	task->cb = cb;
	memcpy(task->va_base, rec->buf, rec->cmd_buf_size);
	task->pa_base = dma_map_single(dev, task->va_base, rec->cmd_buf_size,
				       DMA_TO_DEVICE);
	return task;
}
...
static void cmdq_task_remove_wfe(struct cmdq_task *task)
{
	...
	for (i = 0; i < CMDQ_NUM_CMD(task); i++)
		if ((base[i] & wfe_mask) == (wfe_op | wfe_option))
...
static void cmdq_task_exec(struct cmdq_task *task, struct cmdq_thread *thread)
{
	...
	if (list_empty(&thread->task_busy_list)) {
		 ...
		 * PC = END - CMDQ_INST_SIZE, EOC is executed
...
int cmdq_rec_create(struct device *dev, struct cmdq_rec **rec_ptr)
{
	...
	err = cmdq_rec_realloc_cmd_buffer(rec, PAGE_SIZE);
	if (err < 0) {
		kfree(rec);
		return err;
	}
				   ...
				   u32 arg_a, u32 arg_b)
{
	...
	if (WARN_ON(rec->finalized))
		return -EBUSY;
	if (unlikely(rec->cmd_buf_size + CMDQ_INST_SIZE > rec->buf_size)) {
		err = cmdq_rec_realloc_cmd_buffer(rec, rec->buf_size * 2);
		if (err < 0)
			return err;
	}
	cmd_ptr = rec->buf + rec->cmd_buf_size;
	(*cmd_ptr) = (u64)((code << CMDQ_OP_CODE_SHIFT) | arg_a) << 32 | arg_b;
	rec->cmd_buf_size += CMDQ_INST_SIZE;
	return 0;
}
			...
			struct cmdq_base *base, u32 offset, u32 mask)
{
	...
	if (mask != 0xffffffff) {
		err = cmdq_rec_append_command(rec, CMDQ_CODE_MASK, 0, ~mask);
		if (err < 0)
			return err;
...
int cmdq_rec_wfe(struct cmdq_rec *rec, enum cmdq_event event)
{
	...
	return cmdq_rec_append_command(rec, CMDQ_CODE_WFE,
...
static void cmdq_rec_flush_cb(struct cmdq_cb_data data)
{
...
int cmdq_rec_flush(struct cmdq_client *client, struct cmdq_rec *rec)
{
	...
	err = cmdq_rec_flush_async(client, rec, cmdq_rec_flush_cb, &cmplt);
	if (err < 0)
		return err;
