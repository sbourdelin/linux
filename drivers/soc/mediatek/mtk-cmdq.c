...
#define CMDQ_MAX_TASK_IN_THREAD		20
...
static void cmdq_task_free_command_buffer(struct cmdq_task *task)
{
	...
	dma_free_coherent(task->cmdq->dev, task->command_size, task->va_base,
			  task->mva_base);
	task->va_base = NULL;
	task->mva_base = 0;
	task->command_size = 0;
...
static struct cmdq_task *cmdq_task_create(struct cmdq *cmdq, size_t size)
{
	struct device *dev = cmdq->dev;
	...
	task->cmdq = cmdq;
	task->command_size = size;
	task->va_base = dma_alloc_coherent(dev, size, &task->mva_base,
					   GFP_KERNEL);
	if (!task->va_base) {
					   ...
					   int prev_id)
{
	...
	for (; from_id != prev_id;
		...
		if ((task_base[task->num_cmd - 1] == CMDQ_JUMP_BY_OFFSET) &&
		    (task_base[task->num_cmd - 2] == CMDQ_JUMP_PASS)) {
					   ...
					   struct cmdq_task_cb *cb)
{
	...
	task = cmdq_task_create(cmdq, command->size);
	if (!task) {
...
static int cmdq_task_insert_into_thread(struct cmdq_task *task)
{
	...
	/* find previous task */
	curr = thread->next_cookie % CMDQ_MAX_TASK_IN_THREAD;
	prev = (curr + CMDQ_MAX_TASK_IN_THREAD - 1) % CMDQ_MAX_TASK_IN_THREAD;
	prev_task = thread->cur_task[prev];

	/* cmdq_thread_reorder_task_array() should prevent this error */
	if (WARN_ON(prev_task == NULL))
		return -EFAULT;

	/* insert task, and let previous task jump to this task */
	thread->cur_task[curr] = task;
				...
				struct cmdq_thread *thread)
{
	...
	if (thread->task_count <= 0) {
		...
		if ((curr_pa == end_pa - 8) || (curr_pa == end_pa - 0)) {
			...
			if (thread == &cmdq->thread[0] ||
			    thread == &cmdq->thread[1])
		...
		thread->next_cookie = (thread->next_cookie + 1) %
...
static void cmdq_handle_error(struct cmdq *cmdq, struct cmdq_thread *thread)
{
	 ...
	 * because it wouldn't be error if this interrupt is issued by EOC.
	 * So, we should add 1 to locate "current" task.
	 */
	cookie++;
	...
	/* set the remained tasks to done state */
	cmdq_handle_done(cmdq, thread, cookie);
}
...
static void cmdq_handle_irq(struct cmdq *cmdq, int tid)
{
	...
	if (value & CMDQ_THR_IRQ_ERROR)
		...
		cmdq_handle_done(cmdq, thread, cmdq_thread_get_cookie(thread));
	...
	if (task->num_cmd && thread_pc >= task->mva_base &&
		...
		for (i = 0; i < CMDQ_MAX_TASK_IN_THREAD; i++) {
			...
			if (prev_va[prev_num - 2] == task->mva_base &&
				...
				/* reorder task array to prevent empty slot */
				if (next_task)
...
static int cmdq_remove(struct platform_device *pdev)
{
	...
	/* Destroy work queue safely, so task_active_list must be empty. */
