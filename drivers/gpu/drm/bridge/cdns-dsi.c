...
static int cdns_dsi_bridge_attach(struct drm_bridge *bridge)
{
	...
	case CDNS_DSI_BRIDGE:
		output->bridge->encoder = bridge->encoder;
...
static void cdns_dsi_bridge_detach(struct drm_bridge *bridge)
{
	struct cdns_dsi_input *input = bridge_to_cdns_dsi_input(bridge);
	struct cdns_dsi_output *output = input->dsi->output;

	if (output->type == CDNS_DSI_BRIDGE && output->bridge->dev)
