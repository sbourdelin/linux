...
static int anx78xx_aux_wait(struct anx78xx *anx78xx)
{
	...
	err = regmap_read(anx78xx->map[I2C_IDX_TX_P0], SP_AUX_CH_STATUS_REG,
			  &status);
	if (err || (status & SP_AUX_STATUS)) {
		DRM_ERROR("Failed to read AUX channel: 0x%02x (err: %d)\n",
			  status, err);
		return -ETIMEDOUT;
	}
...
static int anx78xx_dp_link_training(struct anx78xx *anx78xx)
{
	...
	case DP_LINK_BW_5_4:
		break;
	default:
		DRM_DEBUG_KMS("DP bandwidth (0x%02x) not supported\n", dp_bw);
		return -EINVAL;
	}
	...
	if (anx78xx->dpcd[DP_MAX_DOWNSPREAD] & DP_PERCENT_DOWNSPREAD_0_5) {
