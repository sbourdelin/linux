...
static void dispc6_vp_enable(struct dispc_device *dispc,
			     u32 hw_videoport, bool enable)
{
	VP_REG_FLD_MOD(dispc, hw_videoport, DISPC_VP_CONTROL, !!enable, 0, 0);
}
...
static void dispc6_vp_set_lcd_config(struct dispc_device *dispc,
				     u32 hw_videoport,
				     const struct tidss_vp_config *config)
{
	dispc6_set_num_datalines(dispc, hw_videoport, config->video_port_width);
}

static enum drm_mode_status dispc6_lcd_timings_ok(int hsw, int hfp, int hbp,
...
static
enum drm_mode_status dispc6_vp_timings_ok(struct dispc_device *dispc,
					  u32 hw_videoport,
					  const struct videomode *vm)
{
	...
	return dispc6_lcd_timings_ok(vm->hsync_len,
				     vm->hfront_porch, vm->hback_porch,
				     vm->vsync_len,
				     vm->vfront_porch, vm->vback_porch);
}

static void dispc6_vp_set_timings(struct dispc_device *dispc,
				  u32 hw_videoport,
				  const struct videomode *vm)
{
...
static int dispc6_plane_setup(struct dispc_device *dispc, u32 hw_plane,
			      ...
			      bool mem_to_mem, u32 hw_videoport)
{
...
static const struct dispc_ops dispc6_ops = {
	...
	.vp_go = dispc6_vp_go,
	.vp_set_lcd_config = dispc6_vp_set_lcd_config,
	.vp_timings_ok = dispc6_vp_timings_ok,
	.vp_set_timings = dispc6_vp_set_timings,
