...
void tidss_crtc_error_irq(struct drm_crtc *crtc, u64 irqstatus)
{
	...
	dev_err(ddev->dev, "CRTC%u SYNC LOST: (irq %llx)\n",
...
static void tidss_mode_to_vm(const struct drm_display_mode *mode, u32 bus_flags,
			     struct videomode *vm)
{
	drm_display_mode_to_videomode(mode, vm);

	// XXX what should the default values be?
...
struct tidss_bus_format {
	u32 media_bus_fmt;
	u32 data_width;
};

static const struct tidss_bus_format tidss_bus_formats[] = {
...
static int tidss_set_vp_config(struct tidss_vp_config *cfg, u32 bus_format)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(tidss_bus_formats); i++) {
		if (tidss_bus_formats[i].media_bus_fmt != bus_format)
			continue;

		cfg->video_port_width = tidss_bus_formats[i].data_width;
		return 0;
	}

	/* If we did not find a match mode, fall back to sensible default */
	cfg->video_port_width = 24;
				   ...
				   struct drm_crtc_state *state)
{
	...
	tidss_mode_to_vm(mode, tcrtc_state->bus_flags, &vm);

	ok = tidss->dispc_ops->vp_timings_ok(tidss->dispc, tcrtc->hw_videoport,
					     &vm);

	if (ok == MODE_OK)
		return 0;

	dev_err(ddev->dev, "%s: failed (%ux%u pclk %lu)\n",
		__func__, vm.hactive, vm.vactive, vm.pixelclock);
				     ...
				     struct drm_crtc_state *old_state)
{
	...
	tidss->dispc_ops->vp_set_timings(tidss->dispc, tcrtc->hw_videoport,
					 &vm);

	if (tidss_set_vp_config(&cfg, tcrtc_state->bus_format))
		dev_warn(ddev->dev,
			 "%s: unsupported bus format 0x%x, using 18-bit dpi\n",
			 __func__, tcrtc_state->bus_format);

	tidss->dispc_ops->vp_set_lcd_config(tidss->dispc, tcrtc->hw_videoport,
					    &cfg);
	...
	tidss->dispc_ops->vp_enable(tidss->dispc, tcrtc->hw_videoport, true);
				      ...
				      struct drm_crtc_state *old_state)
{
	...
	tidss->dispc_ops->vp_enable(tidss->dispc, tcrtc->hw_videoport, false);
					   ...
					   const struct drm_display_mode *mode)
{
	...
	tidss_mode_to_vm(mode, 0, &vm);

	return tidss->dispc_ops->vp_timings_ok(tidss->dispc,
					       tcrtc->hw_videoport,
					       &vm);
}
