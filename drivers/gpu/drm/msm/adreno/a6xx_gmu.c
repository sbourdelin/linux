...
static int a6xx_gmu_start(struct a6xx_gmu *gmu)
{
	...
	/* Wait for the memory write to post */
	wmb();
...
int a6xx_gmu_set_oob(struct a6xx_gmu *gmu, enum a6xx_gmu_oob_state state)
{
	...
	gmu_write(gmu, REG_A6XX_GMU_GMU2HOST_INTR_CLR, ack);
...
static int a6xx_gmu_fw_start(struct a6xx_gmu *gmu, unsigned int state)
{
	...
	if (state == GMU_WARM_BOOT) {
		...
		/* Load the GMU microcode if we haven't already */
		if (!gmu->ucode) {
			const struct firmware *fw =
				adreno_request_fw(adreno_gpu,
					adreno_gpu->info->powerfw);

			if (IS_ERR(fw))
				return PTR_ERR(fw);

			/* The GMU firmware region is 32k */
			if (fw->size > 0x8000) {
				dev_err(gmu->dev,
		...
		image = (u32 *) gmu->ucode->data;

		for (i = 0; i < gmu->ucode->size >> 2; i++)
			gmu_write(gmu, REG_A6XX_GMU_CM3_ITCM_START + i,
				image[i]);

		/* Wait for the firmware to finish writing */
		wmb();
	}
...
static void a6xx_gmu_disable_gdsc(struct a6xx_gmu *gmu)
{
	if (gmu->cx)
		regulator_disable(gmu->cx);
}

static int a6xx_gmu_enable_gdsc(struct a6xx_gmu *gmu)
{
	int ret = 0;

	if (gmu->cx)
		ret = regulator_enable(gmu->cx);
...
int a6xx_gmu_reset(struct a6xx_gpu *a6xx_gpu)
{
	...
	a6xx_gmu_disable_gdsc(gmu);

	/* Re-enable the resources */
	a6xx_gmu_enable_gdsc(gmu);
...
static int a6xx_gmu_memory_probe(struct a6xx_gmu *gmu)
{
	...
	/*
	 * This is the start of the uncached segment in the GMU memory
	 * map. For now, all shared buffer objects go in here
	 */
...
{
	u32 len = cmd_db_get_aux_data_len(id);
	...
	cmd_db_get_aux_data(id, vals, len);
		...
		u16 *sec, int sec_count)
{
	...
	for (i = 0; i < freqs_count; i++) {
		...
		/* Get the voltage for the OPP level */
		if (freqs[i]) {
			opp  = dev_pm_opp_find_freq_exact(dev, freqs[i], true);
			if (!IS_ERR(opp)) {
				voltage = dev_pm_opp_get_voltage(opp) - 1;
				dev_pm_opp_put(opp);
			}
		}

		/* Get the primary index that matches the voltage level */
		...
		if (j == pri_count) {
			dev_err(dev,
				"Voltage %u not found in in the RPMh levels\n",
				voltage);
			dev_err(dev, "Available voltages:\n");
			for (j = 0; j < pri_count; j++)
				dev_err(dev, "  %u\n", pri[j]);
		...
		/* Look for a voltage in in the secondary list that matches */

		/* If nothing else matches, use the maximum */
		sindex = sec_count - 1;
		...
		/* Construct the vote */
		votes[i] = (voltage << 16) | sindex << 8 | pindex;
	}
...
int a6xx_gmu_probe(struct a6xx_gpu *a6xx_gpu, struct device_node *node)
{
	...
	gmu->cx = devm_regulator_get(gmu->dev, "vddcx");
	...
	gmu->mmio = a6xx_gmu_get_mmio(pdev, "gmu_reg");

	/* Map the GPU power domain controller registers */
	gmu->pdc_mmio = a6xx_gmu_get_mmio(pdev, "gmu_pdc_reg");

	if (IS_ERR(gmu->mmio) || IS_ERR(gmu->pdc_mmio))
		goto err;

	/* Get the HFI and GMU interrupts */
	gmu->hfi_irq = a6xx_gmu_get_irq(gmu, pdev, "hfi_irq", a6xx_hfi_irq);
	gmu->gmu_irq = a6xx_gmu_get_irq(gmu, pdev, "gmu_irq", a6xx_gmu_irq);
