...
{
	if (strncmp(s, "jeida", 5) == 0)
		return LVDS_FORMAT_JEIDA;
	else if (strncmp(s, "vesa", 4) == 0)
			     ...
			     void *data)
{
	...
	if (of_property_read_u32(remote, "rockchip,data-width", &width)) {
		...
		if (width == 24) {
			...
			DRM_DEV_ERROR(dev, "unsupport data-width [%d]\n", width);
			ret = -EINVAL;
			goto err_put_remote;
		}
	}
