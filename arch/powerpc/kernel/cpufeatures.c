...
static int __init feat_enable_hvi(struct dt_cpu_feature *f)
{
	 ...
	 * are always delivered as HVI interrupts rather than EE, so
	 * LPES0 can be set here.
	 */
...
#if 0
XXX: XIVE topic branch adds HEIC (and LPES0 but that may be reverted)
	lpcr |= LPCR_HEIC;	/* disable external interrupts in hypervisor */
#endif
	mtspr(SPRN_LPCR, lpcr);
