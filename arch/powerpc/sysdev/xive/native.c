...
static void xive_native_setup_pools(void)
{
	...
	if (WARN_ON(xive_pool_vps == XIVE_INVALID_VP))
		pr_err("XIVE: Failed to allocate pool VP, "
		       "KVM might not function\n");
...
bool xive_native_init(void)
{
	...
	if (of_address_to_resource(np, 1, &r)) {
		pr_err("XIVE: Failed to get thread mgmnt area resource\n");
		return false;
	}
	tima = ioremap(r.start, resource_size(&r));
	if (!tima) {
		pr_err("XIVE: Failed to map thread mgmnt area\n");
		return false;
	}
