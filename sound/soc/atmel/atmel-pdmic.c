...
static struct atmel_pdmic_pdata *atmel_pdmic_dt_init(struct device *dev)
{
	...
	if (of_property_read_s32(np, "atmel,mic-offset",
				&pdata->mic_offset)) {
		pdata->mic_offset = 0;
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = snd_soc_update_bits(codec, PDMIC_DSPR1,
		PDMIC_DSPR1_DGAIN_MASK,
		mic_gain_table[val].dgain << PDMIC_DSPR1_DGAIN_SHIFT);
	if (ret < 0)
		return ret;

	ret = snd_soc_update_bits(codec, PDMIC_DSPR0,
...
static struct snd_soc_codec_driver soc_codec_dev_pdmic = {
	...
	.get_regmap	= atmel_pdmic_codec_get_remap,
};
