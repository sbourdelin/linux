...
static void da7218_alc_calib(struct snd_soc_codec *codec)
{
	...
	regcache_mark_dirty(da7218->regmap);
	regcache_sync_region(da7218->regmap, DA7218_IN_1_HPF_FILTER_CTRL,
			     DA7218_IN_2_HPF_FILTER_CTRL);
	regcache_sync_region(da7218->regmap, DA7218_IN_1L_FILTER_CTRL,
			     DA7218_IN_2R_FILTER_CTRL);
	regcache_sync_region(da7218->regmap, DA7218_MIXIN_1_CTRL,
			     DA7218_MIXIN_1_CTRL);
	regcache_sync_region(da7218->regmap, DA7218_MIXIN_2_CTRL,
			     DA7218_MIXIN_2_CTRL);
	regcache_sync_region(da7218->regmap, DA7218_MIC_1_CTRL,
			     DA7218_MIC_1_CTRL);
	regcache_sync_region(da7218->regmap, DA7218_MIC_2_CTRL,
			     DA7218_MIC_2_CTRL);

	regcache_cache_bypass(da7218->regmap, false);
}
...
static void da7218_micldet_irq(struct snd_soc_codec *codec)
{
	...
	/* Just one event for voice level detect, simulate press and release */
	input_report_key(da7218->mic_lvl_input, KEY_VOICECOMMAND, 1);
	input_sync(da7218->mic_lvl_input);
	input_report_key(da7218->mic_lvl_input, KEY_VOICECOMMAND, 0);
	input_sync(da7218->mic_lvl_input);
}
...
static enum da7218_ldo_lvl_sel da7218_of_ldo_lvl(struct snd_soc_codec *codec,
						 u32 val)
{
	switch (val) {
	case 1050:
		return DA7218_LDO_LVL_SEL_1_05V;
	case 1100:
		return DA7218_LDO_LVL_SEL_1_10V;
	case 1200:
		return DA7218_LDO_LVL_SEL_1_20V;
	case 1400:
		return DA7218_LDO_LVL_SEL_1_40V;
	default:
...
static int da7218_of_biquad_cfg(struct snd_soc_codec *codec, const char *name,
				u8 **biquad_cfg)
{
	struct device_node *np = codec->dev->of_node;
	int num_items, of_len, i;
	const __be32 *of_val;
	u8 *cfg;

	of_val = of_get_property(np, name, &of_len);
	if ((!of_len) && (of_len % DA7218_OF_BIQUAD_CFG_ROW_SIZE)) {
		dev_warn(codec->dev, "Invalid %s property provided\n", name);
		return 0;
	}

	num_items = of_len / sizeof(u32);
	cfg = devm_kzalloc(codec->dev, num_items, GFP_KERNEL);
	if (!cfg) {
		dev_warn(codec->dev, "Failed to allocate memory for %s data\n",
			 name);
		return 0;
	}

	for (i = 0; i < num_items; ++i)
		cfg[i] = (u8) be32_to_cpu(of_val[i]);
	*biquad_cfg = cfg;
...
static struct da7218_pdata *da7218_of_to_pdata(struct snd_soc_codec *codec)
{
	...
	if (of_property_read_u32(np, "dlg,ldo-lvl", &of_val32) >= 0)
		pdata->ldo_lvl_sel = da7218_of_ldo_lvl(codec, of_val32);
	else
		pdata->ldo_lvl_sel = DA7218_LDO_LVL_SEL_1_05V;
	...
	if (of_property_read_u32(np, "dlg,dmic1-clkrate", &of_val32) >= 0)
	...
	if (of_property_read_u32(np, "dlg,dmic2-clkrate", &of_val32) >= 0)
		pdata->dmic2_clk_rate = da7218_of_dmic_clkrate(codec, of_val32);
	else
		pdata->dmic2_clk_rate = DA7218_DMIC_CLK_3_0MHZ;

	pdata->biquad_cfg_len = da7218_of_biquad_cfg(codec, "dlg,biquad-cfg",
						     &pdata->biquad_cfg);
	pdata->st_biquad_cfg_len = da7218_of_biquad_cfg(codec,
							"dlg,st-biquad-cfg",
							&pdata->st_biquad_cfg);
	...
	if (da7218->dev_id == DA7218_DEV_ID) {
		...
		if (of_property_read_u32(hpldet_np, "dlg,jack-rate",
					 &of_val32) >= 0)
		...
		if (of_property_read_u32(hpldet_np, "dlg,jack-threshold",
					 &of_val32) >= 0)
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		if (snd_soc_codec_get_bias_level(codec) == SND_SOC_BIAS_OFF) {
			/* MCLK */
			clk_prepare_enable(da7218->mclk);
...
static void da7218_handle_pdata(struct snd_soc_codec *codec)
{
	...
	if (pdata) {
		...
		case DA7218_LDO_LVL_SEL_1_05V:
		case DA7218_LDO_LVL_SEL_1_10V:
		case DA7218_LDO_LVL_SEL_1_20V:
		case DA7218_LDO_LVL_SEL_1_40V:
			snd_soc_update_bits(codec, DA7218_LDO_CTRL,
					    DA7218_LDO_LEVEL_SELECT_MASK,
					    (pdata->ldo_lvl_sel <<
					     DA7218_LDO_LEVEL_SELECT_SHIFT));
			break;
		}
		 ...
		 * BiQuad Filters:
		 *
		 * Need to enable at least one output filter channel to be able
		 * to program coefficients.
		 */
		if (pdata->biquad_cfg || pdata->st_biquad_cfg)
			snd_soc_update_bits(codec, DA7218_OUT_1L_FILTER_CTRL,
					    DA7218_OUT_1L_FILTER_EN_MASK,
					    DA7218_OUT_1L_FILTER_EN_MASK);

		if (pdata->biquad_cfg) {
			u8 *biquad_cfg = pdata->biquad_cfg;
			int biquad_cfg_len = pdata->biquad_cfg_len;

			for (i = 0; i < biquad_cfg_len;
			     i += DA7218_BIQUAD_CFG_PAIR_SIZE)
				regmap_raw_write(da7218->regmap,
						 DA7218_OUT_1_BIQ_5STAGE_DATA,
						 (biquad_cfg + i),
						 DA7218_BIQUAD_CFG_PAIR_SIZE);
		}

		if (pdata->st_biquad_cfg) {
			u8 *biquad_cfg = pdata->st_biquad_cfg;
			int biquad_cfg_len = pdata->st_biquad_cfg_len;
		...
		/* Disable output filter, if previously enabled */
		if (pdata->biquad_cfg || pdata->st_biquad_cfg)
			snd_soc_update_bits(codec, DA7218_OUT_1L_FILTER_CTRL,
...
static int da7218_probe(struct snd_soc_codec *codec)
{
	...
	if (da7218->irq) {
		/* Add input for mic level detection */
		input_dev = devm_input_allocate_device(codec->dev);
		if (!input_dev) {
		...
		input_dev->name = "DA7218 Mic Level Detect";
		input_dev->phys = dev_name(codec->dev);
		input_dev->id.bustype = BUS_I2C;
		input_dev->dev.parent = codec->dev;

		input_set_capability(input_dev, EV_KEY, KEY_VOICECOMMAND);

		ret = input_register_device(input_dev);
		if (ret) {
			dev_err(codec->dev, "Failed to register input device\n");
			goto err_disable_reg;
		}

		da7218->mic_lvl_input = input_dev;
