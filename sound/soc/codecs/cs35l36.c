				...
				struct cs35l36_platform_data *pdata)
{
	...
	struct cs35l36_irq_cfg *irq_gpio_config = &pdata->irq_config;
...
{
	struct cs35l36_platform_data *pdata = &cs35l36->pdata;
	struct cs35l36_irq_cfg *irq_config = &pdata->irq_config;
	int irq_polarity;

	if (!irq_config->is_present) {
		dev_err(cs35l36->dev, "IRQ config not present\n");
		return -EINVAL;
	}

	switch (irq_config->irq_pol) {
	...
	regmap_update_bits(cs35l36->regmap, CS35L36_PAD_INTERFACE,
			   ...
			   irq_polarity << CS35L36_INT_POL_SEL_SHIFT);
	regmap_update_bits(cs35l36->regmap, CS35L36_PAD_INTERFACE,
			      ...
			      const struct i2c_device_id *id)
{
	...
	ret  = cs35l36_irq_gpio_config(cs35l36);
	if (ret != 0) {
		dev_err(dev, "IRQ config invalid\n");
		goto err;
	}

	irq_pol = pdata->irq_config.irq_pol;
