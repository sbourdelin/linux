...
static int imx_wm8958_hw_free(struct snd_pcm_substream *substream)
{
	...
	if (!data->is_stream_in_use[id][!tx]) {
		/*
		 * We should connect AIFxCLK source to FLL after enable FLL, and
		...
		if (card->dapm.bias_level == SND_SOC_BIAS_OFF) {
			...
			for (i = 0; i < WM8958_MCLK_MAX; i++) {
				if (!IS_ERR(data->mclk[i])) {
					...
					if (ret)
						dev_warn(card->dev, "Failed to enable MCLK%d: %d\n", i + 1, ret);
				}
			}
		}
		break;
...
static int imx_wm8958_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < DAI_LINK_NUM; i++) {
		sprintf(tmp, "aif%d-", i + 1);
		dai_fmt = snd_soc_of_parse_daifmt(np, tmp, NULL, NULL);

		if (!(dai_fmt & SND_SOC_DAIFMT_FORMAT_MASK)) {
			dev_warn(&pdev->dev,
				 "failed to get format, default to i2s mode\n");
		...
		data->dai_link[i].dai_fmt = dai_fmt;
	}
