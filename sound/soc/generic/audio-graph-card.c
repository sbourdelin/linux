...
static int graph_for_each_link(struct graph_priv *priv, struct link_dpcm *ld,
...
static int graph_parse_of(struct graph_priv *priv)
{
	...
	for (ld.cpu = 1; ld.cpu >= 0; ld.cpu--) {
		 ...
		 */
		ret = graph_for_each_link(priv, &ld,
					  graph_dai_link_of,
