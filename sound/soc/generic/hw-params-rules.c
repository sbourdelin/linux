	...
	struct list_head *list_head)
{
	...
	/* read priority */
	ret = of_property_read_u32(node, "priority", &rule->priority);
	if (ret) {
		dev_err(dev, "%s: can not read \"priority\" - %d\n",
			rule->name, ret);
		return ret;
	}
	...
	struct list_head *list_head)
{
	...
	/* and sort by name */
	list_sort(dev, list_head, asoc_generic_hw_params_rules_sort);
