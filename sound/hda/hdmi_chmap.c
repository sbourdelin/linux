...
EXPORT_SYMBOL_GPL(snd_hdmi_init_channel_allocations);
...
EXPORT_SYMBOL_GPL(snd_hdmi_get_channel_allocation_order);

int snd_hdmi_get_active_channels(int ca)
{
	return channel_allocations[ca].channels;
}
EXPORT_SYMBOL_GPL(snd_hdmi_get_active_channels);

struct cea_channel_speaker_allocation *snd_hdmi_get_ch_alloc_from_ca(int ca)
{
	return &channel_allocations[snd_hdmi_get_channel_allocation_order(ca)];
}
EXPORT_SYMBOL_GPL(snd_hdmi_get_ch_alloc_from_ca);
...
EXPORT_SYMBOL_GPL(snd_hdmi_channel_allocation);
				       ...
				       int ca)
{
	...
	order = snd_hdmi_get_channel_allocation_order(ca);
	...
	for (i = 0; i < 8; i++) {
		...
		err = chmap->ops.pin_set_slot_channel(chmap->hdac, pin_nid, hdmi_slot, channel);
		if (err) {
...
EXPORT_SYMBOL_GPL(snd_hdmi_to_spk_mask);

/* from ALSA API channel position to CEA slot */
static int to_cea_slot(int ordered_ca, unsigned char pos)
{
	int mask = snd_hdmi_to_spk_mask(pos);
	int i;
...
EXPORT_SYMBOL_GPL(snd_hdmi_spk_to_chmap);
...
static int from_cea_slot(int ordered_ca, unsigned char slot)
{
	...
	return snd_hdmi_spk_to_chmap(mask);
}
...
int snd_hdmi_manual_channel_allocation(int chs, unsigned char *map)
{
	...
	for (i = 0; i < chs; i++) {
		int mask = snd_hdmi_to_spk_mask(map[i]);
...
EXPORT_SYMBOL_GPL(snd_hdmi_manual_channel_allocation);
...
static int hdmi_manual_setup_channel_mapping(struct hdmi_chmap *chmap,
					     ...
					     int ca)
{
	int ordered_ca = snd_hdmi_get_channel_allocation_order(ca);
	...
	for (hdmi_slot = 0; hdmi_slot < 8; hdmi_slot++) {
		...
		err = chmap->ops.pin_set_slot_channel(chmap->hdac, pin_nid, hdmi_slot,
...
static void hdmi_setup_fake_chmap(unsigned char *map, int ca)
{
	...
	for (i = 0; i < 8; i++) {
		if (i < channel_allocations[ordered_ca].channels)
			map[i] = from_cea_slot(ordered_ca,
					hdmi_channel_mapping[ca][i] & 0x0f);
		else
			map[i] = 0;
	}
}
...
EXPORT_SYMBOL_GPL(snd_hdmi_setup_channel_mapping);
...
EXPORT_SYMBOL_GPL(snd_hdmi_pin_set_slot_channel);
...
EXPORT_SYMBOL_GPL(snd_hdmi_pin_get_slot_channel);
...
EXPORT_SYMBOL_GPL(snd_hdmi_set_channel_count);
...
EXPORT_SYMBOL_GPL(snd_hdmi_chmap_cea_alloc_validate_get_type);

void snd_hdmi_cea_alloc_to_tlv_chmap(struct cea_channel_speaker_allocation *cap,
					unsigned int *chmap, int channels)
{
	...
	for (c = 7; c >= 0; c--) {
		...
		chmap[count++] = snd_hdmi_spk_to_chmap(spk);
	}

	WARN_ON(count != channels);
}
EXPORT_SYMBOL_GPL(snd_hdmi_cea_alloc_to_tlv_chmap);
