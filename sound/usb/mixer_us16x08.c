	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (err > 0) {
		...
		usb_audio_err(chip, "Failed to set master, err:%d\n", err);
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (err > 0) {
		...
		usb_audio_err(chip, "Failed to set buss param, err:%d\n", err);
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (err > 0) {
		...
		usb_audio_err(chip, "Failed to set channel, err:%d\n", err);
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (err > 0) {
		...
		usb_audio_err(chip, "Failed to set compressor, err:%d\n", err);
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (err > 0) {
		...
		usb_audio_err(chip, "Failed to set eq switch, err:%d\n", err);
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (err > 0) {
		...
		usb_audio_err(chip, "Failed to set eq param, err:%d\n", err);
	}
